<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_master_raport_foreman_front extends CI_Model {

		var $col_order1  = array(null, 'parameter');
		var $col_search1 = array('parameter'); 
		var $order1      = array('parameter' => 'ASC');

		var $col_order2  = array(null, 'a.keterangan');
		var $col_search2 = array('a.keterangan'); 
		var $order2      = array('a.keterangan' => 'ASC');
	    ...
	    function list_parameter(){
			$query = $this->db->select('id, parameter, keterangan')
				->from('mst_raport_foreman_front_hdr')
	    ...
	    private function _get_data_hdr($site){
	    	...
	    	$this->db->select('parameter, keterangan, status_active');
	        $this->db->from('mst_raport_foreman_front_hdr');
	        $this->db->where($datax);
	        $i = 0;
	    ...
	    private function _get_data_dtl($site){
	    	...
	    	$this->db->select('a.keterangan, b.id, b.batas_atas, b.batas_bawah, b.dynamic_value, b.nilai, b.status_active');
	        $this->db->from('mst_raport_foreman_front_hdr a');
	        $this->db->join('mst_raport_foreman_front_dtl b', 'a.id = b.id_raport_foreman_front_hdr', 'inner');
	        $this->db->where($datax);
	        $i = 0;
