<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysphysical_results_obj_kpi extends CI_Controller{
        ...
        function __construct(){
            ...
            if ($this->session->userdata('idu') == null && $this->session->userdata('nik') == null){
                ...
                $this->accessRights = $this->mod_global->get_access_rights($this->session->userdata('idu'), $this->uri->segment(3), $this->pregReps('physical_results_obj_kpi'));
            ...
            $this->load->model(['trans/mphysicalobjkpi/mod_physical_results_obj_kpi']);
        }
        ...
        public function physical_results_obj_kpi($site){
            $data = array(
                'header' => 'pages/ext/header',
                'footer' => 'pages/ext/footer',
                'menu'   => $this->mod_global->menu($this->accessRights->id_user, $site),
                'list_site'  => $this->mod_global->list_site(),
                'content' => 'pages/trans/pphysicalobjkpi/vphysical_results_obj_kpi',
        ...
        public function table_physical_results_obj_kpi($site){
            $data   = array();
            $start  = $this->pregRepn($this->input->post('start'));
            $length = $this->pregRepn($this->input->post('length'));
            if ($start == null || $length == null || $start == "" || $length == ""){ show_404('',false);exit(); }
            $getdata = $this->mod_physical_results_obj_kpi->get_data_physical_results_obj_kpi($length, $start, $site);
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_physical_results_obj_kpi->count_all($site),
                "recordsFiltered" => $this->mod_physical_results_obj_kpi->count_filtered($site),
                "data"            => $data,
            );
