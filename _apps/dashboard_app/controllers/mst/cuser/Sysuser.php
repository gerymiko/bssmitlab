    ...
    class Sysuser extends CI_Controller{
        ...
        function __construct(){
            ...
            if ($this->session->userdata('idu') == null && $this->session->userdata('nik') == null){
                ...
                $this->accessRights = $this->mod_global->get_access_rights($this->session->userdata('idu'), $this->uri->segment(3), $this->pregReps('master_user'));
            ...
            $this->load->model(['mst/muser/mod_user', 'msms/mod_sms']);
        }
        ...
        public function table_user($site){
            ...
            foreach ($user as $field){
                if ($field->status_active == 1){
                    ...
                    $status = '<span class="text-red">Inactive</span>';
                }
                ...
                $row['status'] = $status;
                $row['module'] = '
                    <button class="mdl-button mdl-js-button mdl-button--icon mdl-button--raised mdl-js-ripple-effect button--colored-teal" data-toggle="modal" data-target="#modal-edit-user-module" data-nik="'.$field->nik.'" data-id_user="'.$this->my_encryption->encode($field->id_user).'" onclick="getModule(\''.$this->my_encryption->encode($field->id_user).'\');">
                        <i class="material-icons f18">add</i>
                    </button>';
                $row['action'] = '
                    <button class="mdl-button mdl-js-button mdl-button--icon mdl-button--raised mdl-js-ripple-effect" data-toggle="modal" data-target="#modal-edit-user" data-id_user="'.$this->my_encryption->encode($field->id_user).'" data-fullname="'.$field->fullname.'" data-mobile="'.$field->phone.'" data-email="'.$field->email.'" data-active="'.$field->status_active.'" data-nik='.$field->nik.' data-nik="'.$field->nik.'" data-level="'.$field->id_level.'">
                                ...
                                if ($this->accessRights->id_level == 1) {
                                    foreach ($getModule as $row){
                                        ...
                                        <div class="box-body" id="cbmodule'.$i.'">';
                                            $access = array('createx' => 1, 'readx' => 1, 'updatex' => 1, 'deletex' => 1, 'importx' => 1);
                                    ...
                                    foreach ($getModule as $row){
                                        if ($row->name !== 'master_level_user' && $row->name !== 'master_site' && $row->name !== 'master_module' && $row->name !== 'master_system' ){
                                            echo '
        ...
        public function save_edit_module_user($site){
            ...
            for($i=0; $i < count($dtarray); $i++){
                ...
                foreach($dtarray[$i]['module'] as $id_module => $privVal){
                    ...
                    if ($checkModule === true){
                        ...
                        if (!isset($dataupdate['createx']))
                            $dataupdate['createx'] = 0;
                        if (!isset($dataupdate['read']))
                            $dataupdate['readx'] = 0;
                        if (!isset($dataupdate['updatex']))
                            $dataupdate['updatex'] = 0;
                        if (!isset($dataupdate['deletex']))
                            $dataupdate['deletex'] = 0;
                        if (!isset($dataupdate['import']))
                            $dataupdate['importx'] = 0;
                        ...
                        if (!isset($dataarray['createx']))
                            $dataarray['createx'] = 0;
                        if (!isset($dataarray['readx']))
                            $dataarray['readx'] = 0;
                        if (!isset($dataarray['updatex']))
                            $dataarray['updatex'] = 0;
                        if (!isset($dataarray['deletex']))
                            $dataarray['deletex'] = 0;
                        if (!isset($dataarray['importx']))
                            $dataarray['importx'] = 0;
            ...
            $dataupdatex = array('status_active' => 0, 'createx' => 0, 'readx' => 0, 'updatex' => 0, 'deletex' => 0, 'importx' => 0);
            $editModLast = $this->mod_user->edit_not_in($id_user, 'id_user', $idpriv, 'mst_user_module', $dataupdatex);
            $dataLog = array(
            ...
            if ($nik == null || $nik == '') {
                ...
                echo '
                    ...
                    <div class="form-group hidden">
                        ...
                        <input type="text" name="dept" class="form-control required" value="'.$this->pregReps($getkrywn->KodeDP).'">
                    </div>
        ...
        public function save_add_user($site){
            ...
            $data = array(
                ...
                'dept' => $this->pregReps($this->input->post('dept'))
            );
