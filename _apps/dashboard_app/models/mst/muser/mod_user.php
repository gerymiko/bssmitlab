<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_user extends CI_Model {
        ...
        private function _get_user(){
	        ...
	        $this->db->join('master_user_log b', 'b.id_user = a.id_user', 'left');
	        $this->db->join('master_level_dtl c', 'a.idx = c.idx', 'inner');
	        $this->db->join('master_level_hdr d', 'c.id_level = d.id_level', 'inner');
	    ...
	    function check_module_user_exist($id_user, $id_module, $site){
			$datax = array('id_user' => $this->pregRepn($id_user), 'id_module' => $this->pregRepn($id_module), 'site' => $site);
			$query = $this->db->from('master_user_module')
				->where($datax)
				->get();
			if($query->num_rows() > 0 )
	    ...
	    function list_level(){
	    	...
	    	$query = $this->db->select('b.id_level, b.name, b.status_active')
    			->from('master_level_hdr a')
    			->join('master_level_dtl b', 'a.id_level = b.id_level', 'inner')
    			->where($datax)
    			->get()
    			->result();
	    	return $query;
	    ...
	    function edit_module_user($id_user, $id_module, $site, $data){
			...
			$this->db->update('master_user_module', $data);
