<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_resume_physical extends CI_Model {
	    ...
	    private function _get_data_resume_physical_results($site){
	  ...
	  //   	if($this->pregReps($this->input->post('guideline_policy')))
	        ...
	        $this->db->from('resume_hasil_fisik_obj');
	        $this->db->where($datax);
	        $i = 0;
	    ...
	    function get_data_resume_physical_results($length, $start, $site){
	        $this->_get_data_resume_physical_results($site);
