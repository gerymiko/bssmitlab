<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysoffice extends CI_Controller{

        function __construct(){
            parent::__construct();
            $this->load->model(['mdetail/office/mod_detail_office', 'mglobal/mod_global']);
        ...
        public function detail($site){
            ...
            $data = array(
                ...
                'content'  => 'pages/pdetail/office/view',
                'detailOFF' => $this->mod_detail_office->detail_office($site, $nolambung),
        ...
        public function table_detail_office($site, $nolambung){
            $nolambung = $this->my_encryption->decode($this->uri->segment(4));
            $data   = array();
            $start  = $this->pregRepn($this->input->post('start'));
            $length = $this->pregRepn($this->input->post('length'));
            if ($start == null || $length == null || $start == "" || $length == ""){ show_404();exit(); }
            $detail_office = $this->mod_detail_office->get_detail_office($length, $start, $site, $nolambung);

            foreach ($detail_office as $field){
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_detail_office->count_all_detail_office($site, $nolambung),
                "recordsFiltered" => $this->mod_detail_office->count_filtered_detail_office($site, $nolambung),
                "data"            => $data
            );
