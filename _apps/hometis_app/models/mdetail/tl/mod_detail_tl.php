<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_detail_tl extends CI_Model {

		var $col_order_tl  = array(null, 'c.no_unit', 'd.name', 'a.status_engine', 'a.time_start', 'a.time_end', 'a.hm_start_decimal', 'a.hm_end_decimal', 'b.site');
		var $col_search_tl = array('c.no_unit'); 
		var $order_tl      = array('c.no_unit' => 'ASC');
	    ...
	    function detail_tl($site, $nolambung){
	    ...
	    private function _get_detail_tl($site, $nolambung){
			...
			if ($date_range == null || $date_range == "") {
				$this->db->where('DATEPART(YEAR, a.time_start) =', ''.date("Y").'');
			} else {
				$date_startz = $this->serverDate($date_start);
				$date_endz   = $this->serverDate($date_end);
	        	$this->db->where('CONVERT(VARCHAR, a.time_start, 23) BETWEEN \''.$date_startz.'\' AND \''.$date_endz.'\' ');
			}
	        $i = 0;
	    ...
	    function get_detail_tl($length, $start, $site, $nolambung){
	        $this->_get_detail_tl($site, $nolambung);
	    ...
	    function count_filtered_detail_tl($site, $nolambung){
	        $this->_get_detail_tl($site, $nolambung);
	        $query = $this->db->get();
	        return $query->num_rows();
	    }

	    function count_all_detail_tl($site, $nolambung){
	    	$this->_get_detail_tl($site, $nolambung);
