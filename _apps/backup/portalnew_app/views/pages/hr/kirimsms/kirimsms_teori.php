<h4 style="margin-top: 0px;"><span class="label label-info">Kirim SMS</span> Interview Teori <small>- <i>Data keseluruhan pelamar yang dapat diproses ke tahap interview teori.</i></small></h4>
<hr>
<div class="panel panel-gray" data-collapsed="0">
	<div class="panel-body">
        			...
        			<div class="container-fluid">
			            <div class="form-group">
			            	<div class="tile-stats tile-neon-red" style="height: 114px; padding: 5px 10px 0 20px; margin-bottom: 0px;">
								...
								<div class="num" data-start="0" data-end="<?=$totalPelamarTeori;?>" data-postfix="" data-duration="1400" data-delay="0"><?=$totalPelamarTeori;?></div>
								<h3>Pelamar</h3>
								<p>Tahap tes Teori</p>
							</div>
        				</div>
        				<div class="form-group">
	        				<a class="btn btn-red btn-icon" id="ksmsteori">
								Kirim SMS
		...
		<div style="white-space: nowrap; height: auto; overflow-x: scroll; overflow-y: hidden; cursor: pointer;">
			<table class="table table-hover table-bordered" id="tablePelamarSmsTeori" style="background: #FFF;">
				<thead>
					<tr>
...
<div id="modal-sms-teori" class="modal fade all-modals modal-gray" role="dialog">
	<div class="modal-dialog modal60">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h3 class="modal-title"><span class="label label-info">Kirim SMS Interview TEORI</span></h3>
			</div>
			<form role="form" method="post" id="sendingsmsteori" name="sendingsms" class="validate">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="hidden" name="rs_id" value="3">
								<input type="hidden" name="people_id" id="checkidteori">
								<input type="hidden" name="pelamar_id" id="checkpidteori">
								<input type="hidden" name="people_mobile" id="checkmobileteori">
							</div>
							<div class="form-group">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="side-by-side clearfix">
			                        <select name="schedule_location" id="schedule_locationteori" class="js-example-theme-multiple required" maxlength="50" tabindex="2" data-validate="required" disabled="disabled">
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($city as $row) {
							...
							<div class="form-group">
								<div class="side-by-side clearfix">
			                        ...
			                        <select name="pic_teori" id="pic_teori5" class="js-example-theme-multiple required" maxlength="50" tabindex="2" data-validate="required" >
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($pic_teori as $row) {
...
<script type="text/javascript">
	...
	$(document).ready(function() {
    	table = $('#tablePelamarSmsTeori').DataTable( {
    		"processing" : true,
			"serverSide" : true,
			"stateSave": true,
			"order": [],
			"ajax" : {
				"url"  : '<?php echo site_url('dtKirimSmsTeori')?>',
				"type" : "POST",
		    	"data" : function ( data ) {
	...
	$(document).ready(function() {
		$('#schedule_locationteori').select2();
		$('#pic_teori5').select2();
	});

	function kirimsms(){
		var table = $('#tablePelamarSmsTeori').DataTable();
		var paramstr = $("#sendingsmsteori").serialize();
		if($("#sendingsmsteori").valid() == false){
			return false;
		} else {
			$.post("<?php echo base_url();?>kirimsmspelamarteori",
			paramstr,
			function(data) {
				if(data == "Success"){
					$('#modal-sms-teori').modal('hide');
					table.ajax.reload();
	...
	$(document).ready(function() {
		$('#ksmsteori').click(function () {
		    if (!$("input[name='people_id']").is(':checked')) {
				...
				$('#modal-sms-teori').modal('show').on('shown.bs.modal', function() {
					$("#checkpidteori").val(pid.join(", "));
					$("#checkidteori").val(id.join(", "));
					$("#checkmobileteori").val(mobile.join(", "));
					$("#getnameteori").html(name.join(", "));
					$('#schedule_locationteori').prop('disabled', false);
				});
		    }
		});
	});
