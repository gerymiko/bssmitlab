<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syskirimsmsteori extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/kirimsms/mod_kirimsms_teori', 'hrDept/mod_hr_global', 'hrDept/mod_hr_sms', 'mod_master']);
        }

        public function table_kirimsms_teori(){
            $pelamar_teori = $this->mod_kirimsms_teori->get_datatables();
            $data           = array();
            $no             = $this->input->post('start');

            foreach ($pelamar_teori as $field) {
                $pelamar_id      = $field->pelamar_id;
                $status_kirimsms = $this->mod_kirimsms_teori->status_kirimsms($pelamar_id);
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_kirimsms_teori->count_all(),
                "recordsFiltered" => $this->mod_kirimsms_teori->count_filtered(),
                "data"            => $data,
            );
            echo json_encode($output);
        }

        public function kirimsms_teori(){
            $data = array(
                'sheader'           => 'pages/ext/sheader',
                'sfooter'           => 'pages/ext/sfooter',
                'city'              => $this->mod_master->city(),
                'pic_teori'         => $this->mod_kirimsms_teori->pic_teori(),
                'totalPelamarTeori' => $this->mod_kirimsms_teori->count_all()
            );
            $this->load->view('pages/hr/kirimsms/kirimsms_teori', $data);
        }
        ...
        public function kirimsmspelamar(){
            ...
            $cekstatuspic = $this->mod_kirimsms_teori->cekstatuspic($pic_teori, $rs_id);
            $getmsg       = $this->mod_kirimsms_teori->getcontentmsg();
