<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syskirimsmskspm extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/kirimsms/mod_kirimsms_kspm', 'hrDept/mod_hr_global', 'hrDept/mod_hr_sms', 'mod_master']);
        }

        public function table_kirimsms_kspm(){
            $pelamar_kpsm = $this->mod_kirimsms_kspm->get_datatables();
            $data         = array();
            $no           = $this->input->post('start');
            
            foreach ($pelamar_kpsm as $field) {
                $pelamar_id      = $field->pelamar_id;
                $status_kirimsms = $this->mod_kirimsms_kspm->status_kirimsms($pelamar_id);
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_kirimsms_kspm->count_all(),
                "recordsFiltered" => $this->mod_kirimsms_kspm->count_filtered(),
                "data"            => $data,
            );
            echo json_encode($output);
        }

        public function kirimsms_kspm(){
            $data = array(
                'sheader'          => 'pages/ext/sheader',
                'sfooter'          => 'pages/ext/sfooter',
                'city'             => $this->mod_master->city(),
                'pic_kspm'         => $this->mod_kirimsms_kspm->pic_kspm(),
                'totalPelamarKspm' => $this->mod_kirimsms_kspm->count_all(),
            );
            $this->load->view('pages/hr/kirimsms/kirimsms_kspm', $data);
        }
        ...
        public function kirimsmspelamar(){
            ...
            $pic_kspm     = $this->input->post('pic_kspm');
            $rs_id        = $this->input->post('rs_id');
            $cekstatuspic = $this->mod_kirimsms_kspm->cekstatuspic($pic_kspm, $rs_id);
            $getmsg       = $this->mod_kirimsms_kspm->getcontentmsg($rs_id);
