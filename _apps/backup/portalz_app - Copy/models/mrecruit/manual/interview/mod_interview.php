<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_interview extends CI_Model {
	    ...
	    function detail_people_and_interview($id){
	    	...
	    	$array = array(1, 2, 3);
	    	$query = $this->db->select('a.people_id, a.people_fullname, a.people_noreg, b.KodeJB, b.id, b.interview_site, b.berkas, b.hrdteknis, b.teori, b.teori, b.praktek, b.mcu')
	    ...
	    function get_last_step_interview($id){
	    	$datax = array('id' => $this->pregRepn($id) );
	    	$query = $this->db->select('id, people_id, tahap')
		...
		function edit_people($people_id, $data){
			$this->db->where('people_id', $this->pregRepn($people_id));
			$this->db->update('pmanual_applicant', $data);
			return ($this->db->affected_rows() != 1 ) ? false:true;
		}

		function edit_interview($interview_id, $data){
			$this->db->where('id', $this->pregRepn($interview_id));
			$this->db->update('pmanual_interview', $data);
			return ($this->db->affected_rows() != 1 ) ? false:true;
		}

		function edit_selection_berkas($interview_id, $data){
			$this->db->where('id', $this->pregRepn($interview_id));
			$this->db->update('seleksi_berkas', $data);
			return ($this->db->affected_rows() != 1 ) ? false:true;
		}

		function edit_selection_hrd_teknis($interview_id, $data){
			$this->db->where('id', $this->pregRepn($interview_id));
			$this->db->update('seleksi_hrd_teknis', $data);
			return ($this->db->affected_rows() != 1 ) ? false:true;
		}

		function edit_selection_teori($interview_id, $data){
			$this->db->where('id', $this->pregRepn($interview_id));
			$this->db->update('seleksi_teori', $data);
			return ($this->db->affected_rows() != 1 ) ? false:true;
		}

		function edit_selection_praktek($interview_id, $data){
			$this->db->where('id', $this->pregRepn($interview_id));
			$this->db->update('seleksi_praktek', $data);
			return ($this->db->affected_rows() != 1 ) ? false:true;
		}
