<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_medical extends CI_Model{
	    ...
	    private function _get_datatables_query(){
	    	...
	    	if($cond == ""){
	    		$this->db->or_not_like('b.interview_status', '10');
	    	}
	    	if ($cond == "1") {
			...
			if ($cond == "4" ) {
				$this->db->where('b.interview_status', 4);
			}
			...
			$datax = array('a.people_status' => 1);
			$array = array('Lanjut MCU');
			$this->db->select('a.people_id, a.people_fullname, a.people_noreg, c.edutype_name, a.people_gender, e.city, e.address, d.Nama AS jabatan, b.tgl_melamar, b.timestamp,  b.interview_site, b.tahap, b.interview_status, b.interview_desc, a.people_birth_date, b.id, b.berkas, b.hrdteknis, b.teori, b.praktek, b.mcu ');
			...
			$this->db->where($datax);
			$this->db->where_in('b.interview_desc', $array);
	        $i = 0;
	    ...
	    function check_data_mcu($id){
	    	$datax = array('id' => $this->pregRepn($id) );
	    	$query = $this->db->from('seleksi_mcu')
	    		->where($datax)
	    		->get();
	    	if($query->num_rows() > 0 ){
	            return $query->row();
