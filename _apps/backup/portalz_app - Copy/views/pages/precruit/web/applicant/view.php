				...
				<div class="modal-body">
					<div class="row">
						<div class="col-md-6">
		                    ...
		                    <div class="form-group">
								<label class="control-label">PIC</label>
		                        <select class="form-control select2 required" id="pic_teknis" name="pic_teknis">
		                            <option></option>
		                            <?php
		                            	foreach ($listpic as $row){
		                            		echo '<option value="'.$row->pic_id.'">'.$row->pic_name.'</option>';
						...
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label">Lokasi</label>
		                        <select class="form-control select2 required" id="location" name="location">
		                            <option></option>
		                            <?php
		                            	foreach ($listcity as $row){
		                            		echo '<option value="'.$row->city_id.'">'.$row->city_name.' ['.$row->province_name.']</option>';
					...
					<div class="row">
						<div class="col-md-6">
		                    ...
		                    <div class="form-group">
		                        ...
		                        <select class="form-control select2 required" name="clinic_location" id="clinic_location">
		                            <option></option>
		                            <?php
		                            	foreach ($listcity as $row){
		                            		echo '<option value="'.$row->city_id.'">'.$row->city_name.' ['.$row->province_name.']</option>';
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		var table = $('#table_applicant').DataTable({
			...
			"ajax": {
				"url": '<?=site_url()?>recweb/table/applicant',
				"type": 'POST',
				data : function(data){
					...
					data.status_interview = $('#status_interview').val();
	            },
				error: function(data){
					swal({
				        title: "",
		...
		$('#clinic_id').change(function() {
    		...
    		$.ajax({
    			type: "POST",
    			url: "<?=site_url()?>crecruit/web/applicant/sysapplicant/get_clinic_address",
    			data: opt,
    			success:function(data){
    				$("#clinic_address").val(data);
    			}
    		});
    	});
		...
		$("#btn_sms_kspm").click(function (){
			...
			if($("#form-sms-kspm").valid() == false){
				...
				$.post("<?=base_url();?>crecruit/web/message/sysmessage/send_sms_kspm",
				formdata,
				function(data) {
					if(data == "Success"){
						$("#loading").addClass("hidden");
						...
						swal({
							title: "",
							html: '<i class="fas fa-check-circle f40 margin10 text-green"></i><br>Berhasil kirim sms.',
							type: "",
							confirmButtonText: 'Okay',
						}).then(function(){
							$('#form-sms-kspm')[0].reset();
						...
						swal({
							title: "",
							html: '<i class="fas fa-info-circle f40 margin10 text-orange"></i><br>Sudah dikirim sms sebelumnya.',
							type: "",
							confirmButtonText: 'Okay',
						});
					} else {
						...
						swal({
							title: "",
							html: '<i class="fas fa-times-circle f40 margin10 text-red"></i><br>Gagal dikirim. Muat ulang halaman ini dan coba lagi.',
							type: "",
							confirmButtonText: 'Okay',
						});
					}
		...
		$("#btn_sms_teknis").click(function (){
			...
			if($("#form-sms-teknis").valid() == false){
				...
				$.post("<?=base_url();?>crecruit/web/message/sysmessage/send_sms_teknis",
				formdata,
				function(data) {
					if(data == "Success"){
						$("#loading").addClass("hidden");
		...
		$("#btn_sms_mcu").click(function (){
			...
			if($("#form-sms-mcu").valid() == false){
				...
				$.post("<?=base_url();?>crecruit/web/message/sysmessage/send_sms_mcu",
				formdata,
				function(data) {
					if(data == "Success"){
						$("#loading").addClass("hidden");
						...
						swal({
							title: "",
							html: '<i class="fas fa-info-circle f40 margin10 text-orange"></i><br>Ada tahap yang belum dilaksanakan. MCU dapat dilaksanakan ketika semua tahap (Seleksi berkas, Interview HRD & Teknis / Tes Teori / Tes Praktek) telah terpenuhi sesuai dengan persyaratan tahapan loker.',
							type: "",
							confirmButtonText: 'Okay',
						});
					} else {
	...
	function detailApplicant(id){
		...
		$("#extra-content").load("<?=site_url()?>recweb/applicant/view/detail/"+id);
	}
</script>
