<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysapplicant extends CI_Controller{
        ...
        function __construct(){
            ...
            $this->load->model(['mrecruit/manual/applicant/mod_applicant', 'mglobal/mod_global']);
        }
        ...
        public function save_add_applicant(){
            ...
            $people_id = $this->mod_applicant->insert_into_people('pmanual_applicant', $dataPelamar);
            if ($people_id == false) {
                echo "Error";
                exit();
            }
            ...
            $save_address = $this->mod_applicant->insert_all('pmanual_address', $dataAddress);
            ...
            $save_ktp = $this->mod_applicant->insert_all('pmanual_lisence', $dataKTP);
            if ($save_ktp == false) {
                echo "Error";
                exit();
            }
            if ( $this->pregRepn($this->input->post('sim_A')) == 1 ) {
                ...
                $saveA = $this->mod_applicant->insert_all('pmanual_lisence', $dataSIMA);
                if ($saveA == false) {
                    echo "Error";
                    exit();
                }
            ...
            if ( $this->pregRepn($this->input->post('sim_B1')) == 2 ) {
                ...
                $saveB1 = $this->mod_applicant->insert_all('pmanual_lisence', $dataSIMB1);
                if ($saveB1 == false) {
                    echo "Error";
                    exit();
                }
            ...
            if ( $this->pregRepn($this->input->post('sim_B2')) == 3 ) {
                ...
                $saveB2 = $this->mod_applicant->insert_all('pmanual_lisence', $dataSIMB2);
                if ($saveB2 == false) {
                    echo "Error";
                    exit();
                }
            ...
            if ( $this->pregRepn($this->input->post('sim_B1U')) == 4 ) {
                ...
                $saveB1U = $this->mod_applicant->insert_all('pmanual_lisence', $dataSIMB1U);
                if ($saveB1U == false) {
                    echo "Error";
                    exit();
                }
            ...
            if ( $this->pregRepn($this->input->post('sim_B2U')) == 5 ) {
                ...
                $saveB2U = $this->mod_applicant->insert_all('pmanual_lisence', $dataSIMB1U);
                if ($saveB2U == false) {
                    echo "Error";
                    exit();
                }
            ...
            if ($totalData[0] !== "") {
                ...
                $saveExp = $this->mod_applicant->insert_batch('pmanual_experience', $insertData);
                if ($saveExp == false) {
                    echo "Error";
                    exit();
                }
            ...
            if ($totalDataSkill[0] !== "") {
                ...
                $saveSkill = $this->mod_applicant->insert_batch('pmanual_skill', $insertDataSkill);
                if ($saveSkill == false) {
                    echo "Error";
                    exit();
                }
            ...
            $saveInv = $this->mod_applicant->insert_all('pmanual_interview', $dataInterview);
            $datalogs = array(
                'logs_tanggal'    => date('Y-m-d H:i:s'),
                'logs_ip'         => $this->input->ip_address(),
                'logs_modul'      => 'Pelamar Manual',
                'logs_aktifitas'  => 'Tambah',
                'logs_keterangan' => 'Melakukan penambahan data untuk pelamar ID : '.$people_id.' form (Tambah Pelamar)',
        ...
        public function save_edit_personal(){
            ...
            $editPersonal = $this->mod_applicant->edit_all($id, 'pmanual_applicant', $personal);
            if ($editPersonal == true){
                $datalogs = array(
                    ...
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Ubah',
                    'logs_keterangan' => 'Melakukan perubahan data untuk pelamar ID : '.$id.' form (Ubah Personal)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function save_edit_address(){
            ...
            $editAddress = $this->mod_applicant->edit_all($id, 'pmanual_address', $address);
            if ($editAddress == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Ubah',
                    'logs_keterangan' => 'Melakukan perubahan data untuk pelamar ID : '.$id.' form (Ubah Alamat)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function save_edit_lisence(){
            ...
            $editLisence = $this->mod_applicant->edit_extra_all('plisence_id', $id, 'pmanual_lisence', $lisence);
            if ($editLisence == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Ubah',
                    'logs_keterangan' => 'Melakukan perubahan data untuk pelamar ID : '.$id.' form (Ubah SIM)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function save_edit_experience(){
            ...
            $editExperence = $this->mod_applicant->edit_extra_all('pexp_id', $id, 'pmanual_experience', $experience);
        ...
        public function save_edit_skill(){
            ...
            $editSkill = $this->mod_applicant->edit_extra_all('pskill_id', $id, 'pmanual_skill', $skill);
            if ($editSkill == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Ubah',
                    'logs_keterangan' => 'Melakukan perubahan data untuk pelamar ID : '.$id.' form (Ubah Kemampuan)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function delete_applicant(){
            ...
            $deleteApplicant = $this->mod_applicant->edit_extra_all('people_id', $id, 'pmanual_applicant', $delete);
            if ($deleteApplicant == true){
                $datalogs = array(
                    ...
                    'logs_keterangan' => 'Melakukan perubahan data untuk pelamar ID : '.$id.' form (Hapus Pelamar)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function delete_skill(){
            $id = $this->my_encryption->decode($this->pregReps($this->input->post('id')));
            $deleteSkill = $this->mod_applicant->delete_extra_all('pskill_id', $id, 'pmanual_skill');
            if ($deleteSkill == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
        ...
        public function delete_experience(){
            $id = $this->my_encryption->decode($this->pregReps($this->input->post('id')));
            $deleteExp = $this->mod_applicant->delete_extra_all('pexp_id', $id, 'pmanual_experience');
            if ($deleteExp == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Hapus',
                    'logs_keterangan' => 'Melakukan perubahan data untuk Pengalaman ID : '.$id.' form (Hapus Pengalaman)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function delete_lisence(){
            $id = $this->my_encryption->decode($this->pregReps($this->input->post('id')));
            $deleteLisence = $this->mod_applicant->delete_extra_all('plisence_id', $id, 'pmanual_lisence');
            if ($deleteLisence == true){
                $datalogs = array(
                    ...
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Hapus',
                    'logs_keterangan' => 'Melakukan perubahan data untuk SIM ID : '.$id.' form (Hapus SIM)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function save_add_skill(){
            ...
            $addSkill = $this->mod_applicant->insert_batch('pmanual_skill', $insertDataSkill);
            if ($addSkill == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
        ...
        public function save_add_experience(){
            ...
            $addExp = $this->mod_applicant->insert_batch('pmanual_experience', $insertDataExp);
            if ($addExp == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Tambah',
                    'logs_keterangan' => 'Melakukan penambahan data untuk Pelamar ID : '.$id.' form (Tambah Pengalaman)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
        ...
        public function save_add_lisence(){
            ...
            $addLisence = $this->mod_applicant->insert_all('pmanual_lisence', $lisence);
            if ($addLisence == true){
                $datalogs = array(
                    'logs_tanggal'    => date('Y-m-d H:i:s'),
                    'logs_ip'         => $this->input->ip_address(),
                    'logs_modul'      => 'Pelamar Manual',
                    'logs_aktifitas'  => 'Tambah',
                    'logs_keterangan' => 'Melakukan penambahan data untuk Pelamar ID : '.$id.' form (Tambah SIM)',
                    'logs_user_id'    => $this->session->userdata('bssID'),
                    'logs_username'   => $this->session->userdata('username'),
                    'logs_user_name'  => $this->session->userdata('fullname'),
                    'logs_website'    => 'PORTAL'
