<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class mod_hr_pelamar_monitor_praktek extends CI_Model {
	    ...
	    private function _get_datatables_query(){
			$cond = $this->input->post('tes_praktek');
	    	if($cond == ""){
	    		$this->db->or_not_like('x.tes_praktek', '3');
	    	}
	    	if ($cond == "2") {
				$this->db->where('x.tes_praktek', 2);
			} 
			if ($cond == "1" ) {
				$this->db->where('x.tes_praktek', 1);
			}
			...
			$this->db->distinct();
			$this->db->select('z.people_firstname, z.people_middlename, z.people_lastname, z.schedule_date, g.pic_name, z.jabatan_alias, z.pelamar_id, x.tes_praktek, h.city_name, i.freshgraduate, z.registrant_kode, z.people_id, j.practice_total_result, j.practice_total_answer, j.practice_total_item, z.lowongan_id');
	        ...
	        $this->db->join('(SELECT DISTINCT
							...
							INNER JOIN schedule e ON b.pelamar_id = e.pelamar_id AND e.rs_id = 4) z', 'x.lowongan_id = z.lowongan_id AND x.people_id = z.people_id', 'INNER');
			...
			$this->db->join('practice_result j', 'z.schedule_id = j.schedule_id', 'INNER');
			$this->db->join('practice_result_list k', 'j.practice_r_id = k.practice_r_id', 'LEFT');
	    ...
	    function count_all(){
	    	$this->db->distinct();
			$this->db->select('z.people_firstname, z.people_middlename, z.people_lastname, z.schedule_date, g.pic_name, z.jabatan_alias, z.pelamar_id, x.tes_praktek, h.city_name, i.freshgraduate, z.registrant_kode, z.people_id, j.practice_total_result, j.practice_total_answer, j.practice_total_item');
	        $this->db->from('parameter_job_vacancy x');
	        $this->db->join('(SELECT DISTINCT
								a.people_id,
								e.pelamar_id,
