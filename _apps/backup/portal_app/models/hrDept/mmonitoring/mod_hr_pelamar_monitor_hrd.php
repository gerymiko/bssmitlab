<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class mod_hr_pelamar_monitor_hrd extends CI_Model {
	    ...
	    private function _get_datatables_query(){
	    	$cond = $this->input->post('interview_hrd');
	    	if($cond == ""){
	    		$this->db->or_not_like('x.interview_hrd', '3');
	    	}
	    	if ($cond == "2") {
				$this->db->where('x.interview_hrd', 2);
			} 
			if ($cond == "1" ) {
				$this->db->where('x.interview_hrd', 1);
			}
			...
			$this->db->distinct();
			$this->db->select('z.people_firstname, z.people_middlename, z.people_lastname, z.schedule_date, g.pic_name, z.jabatan_alias, z.pelamar_id, x.interview_hrd, h.city_name, i.freshgraduate, z.registrant_kode, z.people_id, z.lowongan_id');
	        ...
	        $this->db->join('(SELECT DISTINCT
							...
							INNER JOIN schedule e ON b.pelamar_id = e.pelamar_id AND e.rs_id = 5) z', 'x.lowongan_id = z.lowongan_id AND x.people_id = z.people_id', 'INNER');
	    ...
	    function count_all(){
	    	$this->db->distinct();
			$this->db->select('z.people_firstname, z.people_middlename, z.people_lastname, z.schedule_date, g.pic_name, z.jabatan_alias, z.pelamar_id, x.interview_hrd, h.city_name, i.freshgraduate, z.people_id, z.registrant_kode');
	        $this->db->from('parameter_job_vacancy x');
	        $this->db->join('(SELECT DISTINCT
								a.people_id,
								e.pelamar_id,
		...
		function detail_interview_hrd($pelamar_id){
	    	$query = $this->db->from('interview_result a')
	    					->join('schedule b', 'a.schedule_id = b.schedule_id AND b.rs_id = 5', 'INNER')
	    					->join('people_character c', 'a.interview_r_id = c.interview_r_id')
