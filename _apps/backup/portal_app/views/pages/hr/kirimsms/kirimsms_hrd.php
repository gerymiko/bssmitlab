<h4 style="margin-top: 0px;"><span class="label label-info">Kirim SMS</span> Interview KSPM & HRD <small>- <i>Data keseluruhan pelamar yang dapat diproses ke tahap interview KSPM & HRD.</i></small></h4>
<hr>
<div class="panel panel-gray" data-collapsed="0">
	<div class="panel-body">
        			...
        			<div class="container-fluid">
			            <div class="form-group">
			            	<div class="tile-stats tile-neon-red" style="height: 114px; padding: 5px 10px 0 20px; margin-bottom: 0px;">
								...
								<div class="num" data-start="0" data-end="<?=$totalPelamarHrd;?>" data-postfix="" data-duration="1400" data-delay="0"><?=$totalPelamarHrd;?></div>
								<h3>Pelamar</h3>
								<p>Tahap Interview KSPM & HRD</p>
							</div>
        				</div>
        				<div class="form-group">
	        				<a class="btn btn-red btn-icon" id="ksmshrd" onsubmit="idSubmit.disabled = true; return true;">
								Kirim SMS
		...
		<div style="white-space: nowrap; height: auto; overflow-x: scroll; overflow-y: hidden; cursor: pointer;">
			<table class="table table-hover table-bordered" id="tablePelamarSmsHRD" style="background: #FFF;">
				<thead>
					<tr>
...
<div id="modal-sms-hrd" class="modal fade all-modals modal-gray" role="dialog">
	<div class="modal-dialog modal60">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h3 class="modal-title"><span class="label label-info">Kirim SMS Interview KSPM & HRD</span></h3>
			</div>
			<form role="form" method="post" id="sendingsmshrd" name="sendingsms" class="validate">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="hidden" name="people_id" id="checkidhrd">
								<input type="hidden" name="pelamar_id" id="checkpidhrd">
								<input type="hidden" name="people_mobile" id="checkmobilehrd">
							</div>
							<div class="form-group">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="side-by-side clearfix">
			                        <select name="schedule_location" id="schedule_locationhrd" class="js-example-theme-multiple required" maxlength="50" tabindex="2" data-validate="required" disabled="disabled">
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($city as $row) {
							...
							<div class="form-group">
								<div class="side-by-side clearfix">
			                        ...
			                        <select name="pic_hrd" id="pic_hrd2" class="js-example-theme-multiple required" maxlength="50" tabindex="2" data-validate="required" >
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($pic_hrd as $row) {
...
<script type="text/javascript">
	...
	$(document).ready(function() {
    	table = $('#tablePelamarSmsHRD').DataTable( {
    		"processing" : true,
			"serverSide" : true,
			"stateSave": true,
			"order": [],
			"ajax" : {
				"url"  : '<?php echo site_url('dtKirimSmsHRD')?>',
				"type" : "POST",
		    	"data" : function ( data ) {
	...
	$(document).ready(function() {
		$('#schedule_locationhrd').select2();
		$('#pic_hrd2').select2();
	});

	$(document).ready(function() {
		$('#ksmshrd').click(function () {
		    if (!$("input[name='people_id']").is(':checked')) {
				...
				$('#modal-sms-hrd').modal('show').on('shown.bs.modal', function() {
					$("#checkpidhrd").val(pid.join(", "));
					$("#checkidhrd").val(id.join(", "));
					$("#checkmobilehrd").val(mobile.join(", "));
					$("#getnamehrd").html(name.join(", "));
					$('#schedule_locationhrd').prop('disabled', false);
				});
		    }
		});
	});
	...
	function kirimsms(){
		var table = $('#tablePelamarSmsHRD').DataTable();
	 	var paramstr = $("#sendingsmshrd").serialize();
		if($("#sendingsmshrd").valid() == false){
			return false;
		} else {
			$.post("<?php echo base_url();?>kirimsmspelamarhrd",
			paramstr,
			function(data) {
				if(data == "Success"){
					$('#modal-sms-hrd').modal('hide');
					table.ajax.reload();
