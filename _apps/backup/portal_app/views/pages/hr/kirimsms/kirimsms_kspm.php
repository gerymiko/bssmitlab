<h4 style="margin-top: 0px;"><span class="label label-info">Kirim SMS</span> Interview KSPM <small>- <i>Data keseluruhan pelamar yang dapat diproses ke tahap interview KSPM.</i></small></h4>
<hr>
<?php
...
<div class="panel panel-default" data-collapsed="0">
	<div class="panel-body">
        			...
        			<div class="container-fluid">
			            <div class="form-group">
			            	<div class="tile-stats tile-neon-red" style="height: 114px; padding: 5px 10px 0 20px; margin-bottom: 0px;">
								...
								<div class="num" data-start="0" data-end="<?=$totalPelamarKspm;?>" data-postfix="" data-duration="1400" data-delay="0"><?=$totalPelamarKspm;?></div>
        				...
        				<div class="form-group">
	        				<a class="btn btn-red btn-icon" id="ksmskspm">
								Kirim SMS
		...
		<div style="white-space: nowrap; height: auto; overflow-x: scroll; overflow-y: hidden; cursor: pointer;">
			<table class="table table-hover table-bordered" id="tablePelamarSmsKSPM">
				<thead>
					<tr>
...
<div id="modal-sms-kspm" class="modal fade all-modals modal-gray" role="dialog">
	<div class="modal-dialog modal60">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h3 class="modal-title"><span class="label label-info">Kirim SMS Interview KSPM</span></h3>
			</div>
			<form role="form" method="post" id="sendingsmskspm" name="sendingsms" class="validate">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="side-by-side clearfix">
			                        <select name="schedule_location" id="schedule_locationkspm" class="js-example-theme-multiple required select2-offscreen" maxlength="50" tabindex="2" data-validate="required" disabled="disabled">
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($city as $row) {
...
<script type="text/javascript">
	...
	$(document).ready(function() {
    	table = $('#tablePelamarSmsKSPM').DataTable( {
    		"processing" : true,
			"serverSide" : true,
			"order": [],
			"ajax" : {
				"url"  : '<?php echo site_url('dtKirimSmsKSPM')?>',
				"type" : "POST",
		    	"data" : function ( data ) {
	...
	$(document).ready(function() {
		$('#ksmskspm').click(function () {
		    if (!$("input[name='people_id']").is(':checked')) {
				...
				$('#modal-sms-kspm').modal('show').on('shown.bs.modal', function() {
	...
	function kirimsms(){
		var table    = $('#tablePelamarSmsKSPM').DataTable();
		var paramstr = $("#sendingsmskspm").serialize();
		if($("#sendingsmskspm").valid() == false){
			return false;
		} else {
			$.post("<?php echo base_url();?>kirimsmspelamarkspm",
			paramstr,
			function(data) {
				if(data == "Success"){
