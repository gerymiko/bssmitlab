<h4 style="margin-top: 0px;"><span class="label label-info">Kirim SMS</span> Interview Praktek <small>- <i>Data keseluruhan pelamar yang dapat diproses ke tahap interview praktek.</i></small></h4>
<hr>
<div class="panel panel-gray" data-collapsed="0">
	<div class="panel-body">
        			...
        			<div class="container-fluid">
			            <div class="form-group">
			            	<div class="tile-stats tile-neon-red" style="height: 114px; padding: 5px 10px 0 20px; margin-bottom: 0px;">
								...
								<div class="num" data-start="0" data-end="<?=$totalPelamarPraktek;?>" data-postfix="" data-duration="1400" data-delay="0"><?=$totalPelamarPraktek;?></div>
								<h3>Pelamar</h3>
								<p>Tahap Interview Praktek</P>
							</div>
        				</div>
        				<div class="form-group">
	        				<a class="btn btn-red btn-icon" id="ksmspraktek">
								Kirim SMS
		...
		<div style="white-space: nowrap; height: auto; overflow-x: scroll; overflow-y: hidden; cursor: pointer;">
			<table class="table table-hover table-bordered" id="tablePelamarSmsPraktek" style="background: #FFF;">
				<thead>
					<tr>
...
<div id="modal-sms-praktek" class="modal fade all-modals modal-gray" role="dialog">
	<div class="modal-dialog modal60">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h3 class="modal-title"><span class="label label-info">Kirim SMS Interview PRAKTEK</span></h3>
			</div>
			<form role="form" method="post" id="sendingsmspraktek" name="sendingsms" class="validate">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="gray-bg">
									<div class="panel-body">
										<p id="getnamepraktek"></p>
									</div>
								</div>
								<input type="hidden" name="rs_id" value="4">
								<input type="hidden" name="people_id" id="checkidpraktek">
								<input type="hidden" name="pelamar_id" id="checkpidpraktek">
								<input type="hidden" name="people_mobile" id="checkmobilepraktek">
							</div>
							<div class="form-group">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<div class="side-by-side clearfix">
			                        <select name="schedule_location" id="schedule_locationpraktek" class="js-example-theme-multiple required" maxlength="50" tabindex="2" data-validate="required" disabled="disabled">
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($city as $row) {
							...
							<div class="form-group">
								<div class="side-by-side clearfix">
			                        ...
			                        <select name="pic_praktek" id="pic_praktek4" class="js-example-theme-multiple required" maxlength="50" tabindex="2" data-validate="required" >
			                            <option value="">--</option>
			                            <?php
			                            	foreach ($pic_praktek as $row) {
...
<script type="text/javascript">
	...
	$(document).ready(function() {
    	table = $('#tablePelamarSmsPraktek').DataTable( {
    		"processing" : true,
			"serverSide" : true,
			"stateSave": true,
			"order": [],
			"ajax" : {
				"url"  : '<?php echo site_url('dtKirimSmsPraktek')?>',
				"type" : "POST",
		    	"data" : function ( data ) {
            ...
            "fnDrawCallback": function (oSettings) {
        		var i;
				for (i = 0; i <= <?=$totalPelamarPraktek;?>; i++) { 
	...
	$(document).ready(function() {
		$('#schedule_locationpraktek').select2();
		$('#pic_praktek4').select2();
	});

	function kirimsms(){
		var table = $('#tablePelamarSmsPraktek').DataTable();
		var paramstr = $("#sendingsmspraktek").serialize();
		if($("#sendingsmspraktek").valid() == false){
			return false;
		} else {
			$.post("<?php echo base_url();?>kirimsmspelamarpraktek",
			paramstr,
			function(data) {
				if(data == "Success"){
					$('#modal-sms-praktek').modal('hide');
					table.ajax.reload();
	...
	$(document).ready(function() {
		$('#ksmspraktek').click(function () {
		    if (!$("input[name='people_id']").is(':checked')) {
				...
				$('#modal-sms-praktek').modal('show').on('shown.bs.modal', function() {
					$("#checkpidpraktek").val(pid.join(", "));
					$("#checkidpraktek").val(id.join(", "));
					$("#checkmobilepraktek").val(mobile.join(", "));
					$("#getnamepraktek").html(name.join(", "));
					$('#schedule_locationpraktek').prop('disabled', false);
				});
		    }
		});
	});
