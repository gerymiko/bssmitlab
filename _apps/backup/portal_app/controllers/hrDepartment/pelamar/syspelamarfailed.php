<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syspelamarfailed extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/pelamar/mod_hr_pelamar_failed','hrDept/mod_hr_dashboard', 'mod_master',]);
            $this->date_only_def  = date("Y-m-d");
        }

         public function table_pelamar_failed(){
            $pelamar_failed = $this->mod_hr_pelamar_failed->get_datatables();
            $data           = array();
            $no             = $this->input->post('start');
            
            foreach ($pelamar_failed as $field) {
                $no++;
                ...
                // $tglupdate = date("Y-m-d", strtotime($field->tgl_update));
                // $tglgagal = ($tglupdate == $this->date_only_def) ? $baru = "<span class='badge badge-secondary badge-roundless'>Baru</span>" : $baru = "";
                ...
                $row[]  = $field->keterangan_gagal;
                $row[]  = ' <a onClick="ajax_load(\''.site_url().'detailPeople/'.$field->people_id.'/'.$field->registrant_kode.'\')" class="btn btn-info btn-xs" id="detail'.$no.'">
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_hr_pelamar_failed->count_all(),
                "recordsFiltered" => $this->mod_hr_pelamar_failed->count_filtered(),
                "data"            => $data,
            );
        ...
        public function pelamar_failed(){
            $data = array(
                    ...
                    'failedtotal'  => $this->mod_hr_dashboard->getpelamargagaltotal(),
            );
            $this->load->view('pages/hr/pelamar/pelamar_failed', $data);
        }
    }
?>