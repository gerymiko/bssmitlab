<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syspelamarunqualify extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/pelamar/mod_hr_pelamar_unqualify','hrDept/mod_hr_dashboard', 'mod_master']);
            $this->date_only_def  = date("Y-m-d");
        }

        public function table_pelamar_unqualify(){
            $pelamar_unqualify = $this->mod_hr_pelamar_unqualify->get_datatables();
            $data              = array();
            $no                = $this->input->post('start');
            
            foreach ($pelamar_unqualify as $field) {
                $no++;
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_hr_pelamar_unqualify->count_all(),
                "recordsFiltered" => $this->mod_hr_pelamar_unqualify->count_filtered(),
                "data"            => $data
            );
            echo json_encode($output);
        }

        public function pelamar_unqualify(){
            $data = array(
            ...
            $this->load->view('pages/hr/pelamar/pelamar_unqualify', $data);
        }
