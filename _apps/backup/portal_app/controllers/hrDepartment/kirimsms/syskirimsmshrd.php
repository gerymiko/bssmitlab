<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syskirimsmshrd extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/kirimsms/mod_kirimsms_hrd', 'hrDept/mod_hr_global', 'hrDept/mod_hr_sms', 'mod_master']);
        }

        public function table_kirimsms_hrd(){
            $pelamar_hrd = $this->mod_kirimsms_hrd->get_datatables();
            $data        = array();
            $no          = $this->input->post('start');
            
            foreach ($pelamar_hrd as $field) {
                $pelamar_id      = $field->pelamar_id;
                $status_kirimsms = $this->mod_kirimsms_hrd->status_kirimsms($pelamar_id);
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_kirimsms_hrd->count_all(),
                "recordsFiltered" => $this->mod_kirimsms_hrd->count_filtered(),
                "data"            => $data,
            );
            echo json_encode($output);
        }

        public function kirimsms_hrd(){
            $data = array(
                    'sheader'         => 'pages/ext/sheader',
                    'sfooter'         => 'pages/ext/sfooter',
                    'city'            => $this->mod_master->city(),
                    'pic_hrd'         => $this->mod_kirimsms_hrd->pic_hrd(),
                    'totalPelamarHrd' => $this->mod_kirimsms_hrd->count_all(),
            );
            $this->load->view('pages/hr/kirimsms/kirimsms_hrd', $data);
        }
        ...
        public function kirimsmspelamar(){
            ...
            $pic_hrd      = $this->input->post('pic_hrd');
            $rs_id        = $this->input->post('rs_id');
            $cekstatuspic = $this->mod_kirimsms_hrd->cekstatuspic($pic_hrd, $rs_id);
            $getmsg       = $this->mod_kirimsms_hrd->getcontentmsg();
