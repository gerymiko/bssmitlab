<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syskirimsmsteknis extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/kirimsms/mod_kirimsms_teknis', 'hrDept/mod_hr_global', 'hrDept/mod_hr_sms', 'mod_master']);
        }

        public function table_kirimsms_teknis(){
            $pelamar_teknis = $this->mod_kirimsms_teknis->get_datatables();
            $data           = array();
            $no             = $this->input->post('start');

            foreach ($pelamar_teknis as $field) {
                $pelamar_id      = $field->pelamar_id;
                $status_kirimsms = $this->mod_kirimsms_teknis->status_kirimsms($pelamar_id);
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_kirimsms_teknis->count_all(),
                "recordsFiltered" => $this->mod_kirimsms_teknis->count_filtered(),
                "data"            => $data,
            );
            echo json_encode($output);
        }

        public function kirimsms_teknis(){
            $data = array(
                'sheader'            => 'pages/ext/sheader',
                'sfooter'            => 'pages/ext/sfooter',
                'city'               => $this->mod_master->city(),
                'pic_teknis'         => $this->mod_kirimsms_teknis->pic_teknis(),
                'totalPelamarTeknis' => $this->mod_kirimsms_teknis->count_all()
            );
            $this->load->view('pages/hr/kirimsms/kirimsms_teknis', $data);
        }
        ...
        public function kirimsmspelamar(){
            ...
            $pic_teknis   = $this->input->post('pic_teknis');
            $rs_id        = $this->input->post('rs_id');
            $cekstatuspic = $this->mod_kirimsms_teknis->cekstatuspic($pic_teknis, $rs_id);
            $getmsg       = $this->mod_kirimsms_teknis->getcontentmsg();
