<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syskirimsmsfreshgrad extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['hrDept/kirimsms/mod_kirimsms_freshgrad', 'hrDept/mod_hr_global', 'hrDept/mod_hr_sms', 'mod_master']);
        }

        public function table_kirimsms_freshgrad(){
            $pelamar_freshgrad = $this->mod_kirimsms_freshgrad->get_datatables();
            $data              = array();
            $no                = $this->input->post('start');

            foreach ($pelamar_freshgrad as $field) {
                $pelamar_id      = $field->pelamar_id;
                $status_kirimsms = $this->mod_kirimsms_freshgrad->status_kirimsms($pelamar_id);

                if ($status_kirimsms == true) {
                    $statuskirimsms = '<i class="entypo-check green"></i><i class="entypo-mail green"></i> Terkirim';
                } else {
                    $statuskirimsms = '<i class="entypo-cancel red"></i><i class="entypo-mail red"></i>';
                }
                ...
                if ($field->jabatan_alias == null && $field->tgl_melamar != null) {
                    $jabatan = "Sudah Interview";
                } elseif($field->jabatan_alias == null){
                    $jabatan = "Belum Melamar";
                } else {
                    $jabatan = $field->jabatan_alias;
                }

                $tgl_melamar = ($field->tgl_melamar == null) ? '-' : date("d/m/Y", strtotime($field->tgl_melamar));
                ...
                $row[]  = $field->city_name;
                $row[]  = $tgl_melamar;
                $row[]  = $field->pelamar_status;
                $row[]  = $field->lowongan_status;
                $row[]  = $statuskirimsms;
            ...
            $output = array(
                ...
                "recordsTotal"    => $this->mod_kirimsms_freshgrad->count_all(),
                "recordsFiltered" => $this->mod_kirimsms_freshgrad->count_filtered(),
                "data"            => $data,
            );
            echo json_encode($output);
        }

        public function kirimsms_freshgrad(){
            $data = array(
                'sheader' => 'pages/ext/sheader',
                'sfooter' => 'pages/ext/sfooter',
                'city'    => $this->mod_master->city(),
                'pic_fg'  => $this->mod_master->pic(),
                'totalPelamarFG' => $this->mod_kirimsms_freshgrad->count_all_one() + $this->mod_kirimsms_freshgrad->count_all_two(),
            );
            $this->load->view('pages/hr/kirimsms/kirimsms_freshgrad', $data);
        }
        ...
        public function kirimsmspelamar(){
            ...
            $pic_teori   = $this->input->post('pic_teori');
            $rs_id        = $this->input->post('rs_id');
            $cekstatuspic = $this->mod_kirimsms_freshgrad->cekstatuspic($pic_teori, $rs_id);
            $getmsg       = $this->mod_kirimsms_freshgrad->getcontentmsg();
