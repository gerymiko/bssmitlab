<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_payload_hd extends CI_Model {
		...
		var $col_search_payload = array('a.unit', 'a.payload', 'a.nik', 'a.nama', 'a.nikoprloader', 'a.nmloader', 'a.loader'); 
	    ...
	    private function _get_payload($sn){
			...
			$this->db->select('a.unit, CONVERT(VARCHAR, a.tgl, 23) as tgl, a.tgl as dates, CONVERT(VARCHAR, a.tgl, 8) as time,  a.payload, a.nik, a.nama, a.loader,  a.nikoprloader, a.nmloader');	
	    ...
	    function max_bcm_monthly($sn){
	    	$datax = array( 'c.serialnumber' => $this->pregReps($sn), 'DATEPART(YEAR, a.tgl) =' => ''.date("Y").'', 'DATEPART(MONTH, a.tgl) =' => ''.date("m").'');
	    	$query = $this->db->select('a.unit, a.payload, a.nik, a.nama, a.loader, a.nikoprloader, a.nmloader')
	    			->from('payload a')
	    			->join('unit c', ' a.unit = c.nolambung AND c.isDelete = 0', 'inner')
	    			->where($datax)
	    			->order_by('a.payload DESC')
	    			->limit(1)
	    			->get();
	    	if ($query->num_rows() > 0){
	            return $query->row();
	    ...
	    function min_bcm_monthly($sn){
	    	$datax = array( 'c.serialnumber' => $this->pregReps($sn), 'DATEPART(YEAR, a.tgl) =' => ''.date("Y").'', 'DATEPART(MONTH, a.tgl) =' => ''.date("m").'');
	    	$query = $this->db->select('a.unit, a.payload, a.nik, a.nama, a.loader, a.nikoprloader, a.nmloader')
	    			->from('payload a')
	    			->join('unit c', ' a.unit = c.nolambung AND c.isDelete = 0', 'inner')
	    			->where($datax)
	    			->order_by('a.payload ASC')
	    			->limit(1)
	    			->get();
	    	if ($query->num_rows() > 0){
	            return $query->row();
	    ...
	    function array_max_bcm_monthly($sn){
	    	$datax = array( 'c.serialnumber' => $this->pregReps($sn), 'DATEPART(YEAR, a.tgl) =' => ''.date("Y").'', 'DATEPART(MONTH, a.tgl) =' => ''.date("m").'');
	    	$query = $this->db->select('a.unit, a.payload, a.nik, a.nama, a.loader, a.nikoprloader, a.nmloader')
	    			->from('payload a')
	    			->join('unit c', ' a.unit = c.nolambung AND c.isDelete = 0', 'inner')
	    ...
	    function array_min_bcm_monthly($sn){
	    	$datax = array( 'c.serialnumber' => $this->pregReps($sn), 'DATEPART(YEAR, a.tgl) =' => ''.date("Y").'', 'DATEPART(MONTH, a.tgl) =' => ''.date("m").'');
	    	$query = $this->db->select('a.unit, a.payload, a.nik, a.nama, a.loader, a.nikoprloader, a.nmloader')
	    			->from('payload a')
	    			->join('unit c', ' a.unit = c.nolambung AND c.isDelete = 0', 'inner')
