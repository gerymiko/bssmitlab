<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_panel extends CI_Model {
		...
		var $col_order_dozer  = array(null, 'unit', 'type_unit', 'serialnumber', 'nolambung');
		var $col_search_dozer = array('unit', 'type_unit', 'serialnumber', 'nolambung'); 
		var $order_dozer      = array('serialnumber' => 'ASC');
	    ...
	    function get_servername_site($sn){
	    	$datax = array('a.nolambung' => $this->pregReps($sn) );
	    ...
	    private function _get_unit_dozer(){
	    	$datax = array('type_unit' => 'Dozer Truck', 'isDelete' => 0, 'status' => 1);
	        $this->db->from('unit');
	        $this->db->where($datax);
	
	        $i = 0;
	        foreach ($this->col_search_dozer as $item){
	            if($this->pregReps($_POST['search']['value'])){
	                ...
	                if(count($this->col_search_dozer) - 1 == $i) $this->db->group_end(); 
	            }
	            $i++;
	        }
	        if(isset($_POST['order'])){
				$this->db->order_by($this->pregReps($this->col_order_dozer[$_POST['order']['0']['column']]), $this->pregReps($_POST['order']['0']['dir']));
			} else if(isset($this->order_dozer)){
				$order = $this->order_dozer;
				$this->db->order_by(key($order), $order[key($order)]);
			}
	    }

	    function get_unit_dozer($length, $start){
	        $this->_get_unit_dozer();
	    ...
	    function count_filtered_unit_dozer(){
	        $this->_get_unit_dozer();
	        $query = $this->db->get();
	        return $query->num_rows();
	    }

	    function count_all_unit_dozer(){
	    	$this->_get_unit_dozer();
	    ...
	    function get_unit_exca($length, $start){
	        $this->_get_unit_exca();
	        if($this->pregReps($length) != -1) {
	        	$this->db->limit($this->pregReps($length), $this->pregReps($start));
		        $query = $this->db->get();
	    ...
	    function get_unit_hd($length, $start){
	        $this->_get_unit_hd();
	        if($this->pregReps($length) != -1) {
	        	$this->db->limit($this->pregReps($length), $this->pregReps($start));
		        $query = $this->db->get();
