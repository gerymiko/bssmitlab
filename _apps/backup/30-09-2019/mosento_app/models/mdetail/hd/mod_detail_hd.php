<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_detail_hd extends CI_Model {
		...
		//OIL MIN
		var $col_order_opmin  = array(null, 'b.unit', 'a.date', 'a.oilplomin');
		var $col_search_opmin = array('b.unit', 'a.date', 'a.oilplomin'); 
		var $order_opmin      = array('a.date' => 'DESC');

		//OIL MIN
		var $col_order_opmax  = array(null, 'b.unit', 'a.date', 'a.oilpmax');
		var $col_search_opmax = array('b.unit', 'a.date', 'a.oilplomin'); 
		var $order_opmax      = array('a.date' => 'DESC');
		...
		//OIL PRESSURE MIN
		private function _get_oil_min($sn){
	    	$date_range = str_replace("/", "-", $this->input->post('date_range'));
			$date_start = substr($date_range, 0, 10);
			$date_end   = substr($date_range, 13, 23);
			$datax      = array( 'b.serialnumber' => $this->pregReps($sn) );

	    	$this->db->select('a.date, a.oilplomin, b.unit');
	        $this->db->from('trend a');
	        $this->db->join('unit b', 'a.unit = b.nolambung AND b.isDelete = 0 AND b.status = 1', 'inner');
	        $this->db->where($datax);
	        if ($date_range == null || $date_range == "") {
	    ...
	    function get_oil_min($sn){
	        $this->_get_oil_min($sn);
	        if($this->pregReps($_POST['length']) != -1)
	        $this->db->limit($this->pregReps($_POST['length']), $this->pregReps($_POST['start']));
	        $query = $this->db->get();
	        return $query->result();
	    ...
	    function count_filtered_oil_min($sn){
	        $this->_get_oil_min($sn);
	        $query = $this->db->get();
	        return $query->num_rows();
	    }

	    function count_all_oil_min($sn){
	    	$this->_get_oil_min($sn);
	        return $this->db->count_all_results();
	    }

	    function get_data_mastervar_oil_min(){
	    	$datax = array( 'a.alias' => 'oilpressmin', 'b.status' => 1 );
			$query = $this->db->select('a.cautionValue, a.criticalValue, b.value, b.operation')
					->from('tmasterVar a')
					->join('tsatuan b', 'a.jenistrans = b.jenistrans', 'inner')
					->where($datax)
	    ...
	    function get_data_oil_press_min($sn){
	    	$datax = array('b.serialnumber' => $this->pregReps($sn) );
		  	$query = $this->db->select('CONVERT (VARCHAR, a.date, 110) as date, a.oilplomin')
		  			->from('trend a')
		  			->join('unit b', 'a.unit = b.nolambung AND b.isDelete = 0 AND b.status = 1', 'inner')
		  			->where($datax)
		  			->where('DATEPART(YEAR, a.date) =', ''.date("Y").'')
		...
		//OIL PRESSURE MAX
		private function _get_oil_max($sn){
	    	$date_range = str_replace("/", "-", $this->input->post('date_range'));
			$date_start = substr($date_range, 0, 10);
			$date_end   = substr($date_range, 13, 23);
			$datax      = array( 'b.serialnumber' => $this->pregReps($sn) );

	    	$this->db->select('a.date, a.oilpmax, b.unit');
	        $this->db->from('trend a');
	        $this->db->join('unit b', 'a.unit = b.nolambung AND b.isDelete = 0 AND b.status = 1', 'inner');
	        $this->db->where($datax);
	        if ($date_range == null || $date_range == "") {
	        ...
	        foreach ($this->col_search_opmax as $item){
	            if($this->pregReps($_POST['search']['value'])){
	                if($i===0){
	                    $this->db->group_start(); 
	                    $this->db->like($item, $this->pregReps($_POST['search']['value']));
	                ...
	                if(count($this->col_search_opmax) - 1 == $i) $this->db->group_end(); 
	            }
	            $i++;
	        }

	        if(isset($_POST['order'])){
				$this->db->order_by($this->pregReps($this->col_order_opmax[$_POST['order']['0']['column']]), $this->pregReps($_POST['order']['0']['dir']));
			} else if(isset($this->order_opmax)){
				$order = $this->order_opmax;
				$this->db->order_by(key($order), $order[key($order)]);
			}
	    }

	    function get_oil_max($sn){
	        $this->_get_oil_max($sn);
	        if($this->pregReps($_POST['length']) != -1)
	        $this->db->limit($this->pregReps($_POST['length']), $this->pregReps($_POST['start']));
	        $query = $this->db->get();
	        return $query->result();
	    ...
	    function count_filtered_oil_max($sn){
	        $this->_get_oil_max($sn);
	        $query = $this->db->get();
	        return $query->num_rows();
	    }

	    function count_all_oil_max($sn){
	    	$this->_get_oil_max($sn);
	        return $this->db->count_all_results();
	    }

	    function get_data_mastervar_oil_max(){
	    	$datax = array( 'a.alias' => 'oilpressmin', 'b.status' => 1 );
			$query = $this->db->select('a.cautionValue, a.criticalValue, b.value, b.operation')
					->from('tmasterVar a')
					->join('tsatuan b', 'a.jenistrans = b.jenistrans', 'inner')
	    ...
	    function get_data_oil_press_max($sn){
	    	$datax = array('b.serialnumber' => $this->pregReps($sn) );
		  	$query = $this->db->select('CONVERT (VARCHAR, a.date, 110) as date, a.oilpmax')
		  			->from('trend a')
		  			->join('unit b', 'a.unit = b.nolambung AND b.isDelete = 0 AND b.status = 1', 'inner')
		  			->where($datax)
		  			->where('DATEPART(YEAR, a.date) =', ''.date("Y").'')
