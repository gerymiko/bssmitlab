<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_karir_job extends CI_Model {
		var $column_order  = array('a.pjobhistory_company');
		var $column_search = array('a.pjobhistory_company'); 
		var $order         = array('a.pjobhistory_company' => 'ASC');
	    ...
	    private function _get_datatables_query($people_id){
	        $this->db->from('people_job_history a');
	        $this->db->join('mbidang b', 'a.pjobhistory_bidang = b.sector_id', 'inner');
	        $this->db->where('a.people_id', $people_id);
	        $this->db->where('a.pjobhistory_status', 1);
	        $this->db->where('b.sector_status', 1);
