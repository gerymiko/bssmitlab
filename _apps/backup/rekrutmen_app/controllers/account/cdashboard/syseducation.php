<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syseducation extends CI_Controller {
        ...
        public function tab_education(){
            ...
            $this->load->view('pages/account/dash/vtab_education', $data);
        }

        public function tab_ijazah(){
            $people_id = $this->session->userdata('people_id');
            $data = array(
                'dedufor'   => $this->mod_karir_education->detail_edu_formal($people_id),
                'deduinfor' => $this->mod_karir_education->detail_edu_informal($people_id),
            ...
            $this->load->view('pages/account/dash/vtab_ijazah', $data);
        }
        ...
        public function save_edit_eduformal(){
            ...
            $people_id = $this->session->userdata('people_id');
            $data = array(
                'edutype_id'      => $this->pregRepn($this->input->post('edutype_id')),
                'edu_name'        => $this->pregReps($this->input->post('edu_name')),
                'edu_jurusan'     => $this->pregRepn($this->input->post('edu_jurusan')),
                'edu_place'       => $this->pregRepn($this->input->post('edu_place')),
                'edu_tahun_lulus' => date("Y-m-d", strtotime($this->input->post('edu_tahun_lulus'))),
                'edu_keterangan'  => $this->pregReps($this->input->post('edu_keterangan')),
        ...
        public function save_add_informal(){
            $data = array(
                ...
                'informaledu_dari'       => date("Y-m-d", strtotime($this->input->post('informaledu_dari'))),
                'informaledu_sampai'     => date("Y-m-d", strtotime($this->input->post('informaledu_sampai'))),
        ...
        public function delete_informal(){
            $id   = $this->pregRepn($this->input->post('informaledu_id'));
                            ...
                            if ($cijazah->plisence_file == NULL) {
                                if(isset($_FILES['file_ijazah']['name'])) {
                                    $ijazah_data = array(
                                        ...
                                        'plisence_date_start'  => date("Y-m-d", strtotime($this->input->post('plisence_date_start_ijazah'))),
        ...
        public function table_ijazah(){
            ...
            foreach ($ijazah as $field) {
                ...
                $nomor  = ($field->plisence_number == NULL) ? "-" : $field->plisence_number;
                ...
                $row[]  = $status;
                $row[]  = '
                            <button class="btn btn-sm btn-gray nomargin noradius" '.$btn_upload.' data-toggle="modal" data-target="#unggahIjazah">Unggah</button>
                            <button class="btn btn-sm btn-danger nomargin noradius" '.$btn_view.' data-toggle="modal" data-src="'.site_url().'account/show/ijazah/'.$id.'" data-toggle="modal" data-target="#showIjazah">Lihat</button>
                        ';
                $data[] = $row;
            };
            $output = array(
        ...
        public function show_ijazah($id){
            $plisence_id = $this->encrypt->decode($id);
            $getFilename = $this->mod_karir_education->get_file_ijazah($plisence_id);
