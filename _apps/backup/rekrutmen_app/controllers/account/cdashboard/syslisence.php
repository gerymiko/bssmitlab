<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Syslisence extends CI_Controller {
        ...
        public function tab_lisence(){
            ...
            $this->load->view('pages/account/dash/vtab_lisence', $data);
        }
        ...
        public function table_lisence(){
            ...
            foreach ($lisence as $field) {
                ...
                $row[]  = $stberkas;
                $row[]  = '
                    <button class="btn nomargin noradius '.$btn_disable_upload.'" data-toggle="modal" data-target="#uploadLisence" data-id="'.$field->plisence_id.'" data-tipe="'.$field->plisence_type.'">Unggah</button>
        ...
        public function save_add_sim(){
            ...
            if (in_array($plisence_type, $array2)) {
                echo "Duplicate";
                exit();
            }
        ...
        public function save_upload_lisence(){
            $people_id   = $this->session->userdata('people_id');
            $plisence_id = $this->pregRepn($this->input->post('plisence_id'));
            $getFile     = $this->mod_karir_lisence->get_file_lisence($plisence_id);
            if ($getFile->plisence_file !== null) {
                echo "Done";
                exit();
            }
                ...
                if(isset($_FILES['file_lisence']['name'])){
                    if($this->upload->do_upload('file_lisence')){
                        ...
                        if(isset($_FILES['file_lisence']['name'])){
                            ...
                            if(file_exists(dirname("E:\\").'/images/karir/upload/'.$getFile->plisence_file) && $getFile->plisence_file !== null )
                            unlink(dirname("E:\\").'/images/karir/upload/'.$getFile->plisence_file);
