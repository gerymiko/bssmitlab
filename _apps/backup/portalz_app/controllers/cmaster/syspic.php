    ...
    class Syspic extends CI_Controller {
        ...
        public function table_pic(){
            ...
            foreach ($getdata as $field){
                ...
                $row['status']   = $status;
                $row['action']   = '
                    <a class="btn bg-gray btn-xs" data-tooltip="Ubah" onClick="btn_edit_cond(\''.$this->my_encryption->encode($field->pic_id).'\')">
                        <i class="fas fa-pen f9"></i>
                    </a>
                    <a class="btn btn-danger btn-xs data-tooltip="Hapus" onClick="removeThis(\''.$this->my_encryption->encode($field->pic_id).'\', \''.$field->pic_name.'\')">
