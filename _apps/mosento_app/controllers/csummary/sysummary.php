<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysummary extends CI_Controller{
        ...
        function __construct(){
            ...
            if ($this->session->userdata('id_user') == null && $this->session->userdata('nik') == null){
                ...
                $this->accessRights = $this->mod_global->get_access_rights($this->session->userdata('id_user'), $this->uri->segment(3), $this->pregReps('summary'));
            ...
            $this->load->model(['msummary/mod_summary']);
        }
        ...
        public function summary($site){
            $data = array(
                'header'  => 'pages/ext/header',
                'footer'  => 'pages/ext/footer',
                'menu'    => $this->mod_global->menu($this->accessRights->id_user, $site),
                'content' => 'pages/psummary/vsummary',
                'accessRights' => $this->accessRights,
                'today' => $this->mod_summary->count_caution_today($site),
                'week' => $this->mod_summary->count_caution_week($site),
                'month' => $this->mod_summary->count_caution_month($site),
                'year' => $this->mod_summary->count_caution_year($site),
                'css_script' => array(
        ...
        // public function caution($site){
        //     $get_data = $this->mod_summary->caution_month($site);
        //     foreach ($get_data as $row) {
        //         $output[] = array(
        //             'date' => $this->viewDate($row->tgl),
        //             'unit' => $row->unit,
        //             'alert' => $row->alert,
        //             'value' => $row->value
        //         );
        ...
        public function table_caution_month($site){
            $data   = array();
            $length = $this->pregRepn($this->input->post('length'));
            $start  = $this->pregRepn($this->input->post('start'));
            if ($start == null || $length == null || $start == "" || $length == ""){ show_404('', false); }
            $caution = $this->mod_summary->get_caution($length, $start, $site);
            foreach ($caution as $field){
                $start++;
                $row          = array();
                $row['no']    = $start;
                $row['unit']  = $field->unit;
                $row['date']  = '<span class="hidden">'.strtotime($field->tgl).'</span>'.$this->viewDate($field->tgl);
                $row['alert'] = $field->alert;
                $row['value'] = $field->value;
                $data[]       = $row;
            };
            $output = array(
                "draw"            => $this->pregRepn($this->input->post('draw')),
                "recordsTotal"    => $this->mod_summary->count_all($site),
                "recordsFiltered" => $this->mod_summary->count_filtered($site),
                "data"            => $data,
            );
            echo json_encode($output);
        }
