<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysexport_exca extends CI_Controller{
        ...
        function export($site){
            ...
            $trend_data = $this->mod_warning_exca->fetch_data_trend($sn, $dateStart, $dateEnd, $site);
            ...
            $tcolumn7 = array('Date', 'Time', 'Pressure (mPa)');
            $tcolumn8 = array('Date', 'Time', 'Pressure (mPa)');
            $tcolumn9 = array('Date', 'Time', 'Pressure (mPa)');
            $tcolumn10 = array('Date', 'Time', 'Pressure (mPa)');
            $tcolumn11 = array('Date', 'Time', 'Pressure (mPa)');
            $tcolumn12 = array('Date', 'Time', 'Temperatures (DegC)');
            $tcolumn13 = array('Date', 'Time', 'Temperatures (DegC)');
            $tcolumn14 = array('Date', 'Time', 'Second (Sec)');
            $tcolumn15 = array('Date', 'Time', 'Second (Sec)');
            $tcolumn16 = array('Date', 'Time', 'Second (Sec)');
            $tcolumn17 = array('Date', 'Time', 'Second (Sec)');
            $tcolumn18 = array('Date', 'Time', 'Second (Sec)');
            ...
            $warning_data = $this->mod_warning_exca->fetch_data_warning($sn, $dateStart, $dateEnd, $site);
            ...
            list($rowmaster1) = $this->mod_warning_exca->fetch_data_mastervar('engoiltemp');
            ...
            list($rowmaster2) = $this->mod_warning_exca->fetch_data_mastervar('fuelrate');
            foreach ($trend_data as $row){
            ...
            list($rowmaster3) = $this->mod_warning_exca->fetch_data_mastervar('tmoiltemp');
            foreach ($trend_data as $row){
            ...
            list($rowmaster4) = $this->mod_warning_exca->fetch_data_mastervar('cooltemp');
            foreach ($trend_data as $row){
            ...
            list($rowmaster5) = $this->mod_warning_exca->fetch_data_mastervar('blowbypress');
            foreach ($trend_data as $row){
            ...
            list($rowmaster6) = $this->mod_warning_exca->fetch_data_mastervar('blowbypress');
            foreach ($trend_data as $row){
            ...
            list($rowmaster7) = $this->mod_warning_exca->fetch_data_mastervar('pumpF_press_max');
            foreach ($trend_data as $row){
                if ($rowmaster7->operation == 'multiplication') {
                    $values7 = (floatval($row->pumpF_press_max) * floatval($rowmaster7->value));
                } elseif ($rowmaster7->operation == 'division') {
                    $values7 = (floatval($row->pumpF_press_max) / floatval($rowmaster7->value));
                } else {
                    $values7 = floatval($row->pumpF_press_max);
                }
            ...
            $objWorkSheet7->setTitle('Pump Front Pressure Maximum');

            // Pump Rear Pressure Maximal
            $column8    = 0;
            $excel_row8 = 2;
            ...
            list($rowmaster8) = $this->mod_warning_exca->fetch_data_mastervar('pumpR_press_max');
            foreach ($trend_data as $row){
                if ($rowmaster8->operation == 'multiplication') {
                    $values8 = (floatval($row->pumpR_press_max) * floatval($rowmaster8->value));
                } elseif ($rowmaster8->operation == 'division') {
                    $values8 = (floatval($row->pumpR_press_max) / floatval($rowmaster8->value));
                } else {
                    $values8 = floatval($row->pumpR_press_max);
                }
            ...
            $objWorkSheet8->setTitle('Pump Rear Pressure Maximal');
            ...
            list($rowmaster9) = $this->mod_warning_exca->fetch_data_mastervar('swing_press_max');
            foreach ($trend_data as $row){
                if ($rowmaster9->operation == 'multiplication') {
                    $values9 = (floatval($row->swing_press_max) * floatval($rowmaster9->value));
                } elseif ($rowmaster9->operation == 'division') {
                    $values9 = (floatval($row->swing_press_max) / floatval($rowmaster9->value));
                } else {
                    $values9 = floatval($row->swing_press_max);
                }
            ...
            $objWorkSheet9->setTitle('Swing Pressure Maximum');

            // G1 Pump Pressure Maximum
            $column10    = 0;
            $excel_row10 = 2;
            ...
            list($rowmaster10) = $this->mod_warning_exca->fetch_data_mastervar('g1pump_press_max');
            foreach ($trend_data as $row){
                if ($rowmaster10->operation == 'multiplication') {
                    $values10 = (floatval($row->g1pump_press_max) * floatval($rowmaster10->value));
                } elseif ($rowmaster10->operation == 'division') {
                    $values10 = (floatval($row->g1pump_press_max) / floatval($rowmaster10->value));
                } else {
                    $values10 = floatval($row->g1pump_press_max);
                }
            ...
            $objWorkSheet10->setTitle('G1 Pump Pressure Maximum');

            // G2 Pump Pressure Maximal
            $column11    = 0;
            $excel_row11 = 2;
            ...
            list($rowmaster11) = $this->mod_warning_exca->fetch_data_mastervar('g2pump_press_max');
            foreach ($trend_data as $row){
                if ($rowmaster11->operation == 'multiplication') {
                    $values11 = (floatval($row->g2pump_press_max) * floatval($rowmaster11->value));
                } elseif ($rowmaster11->operation == 'division') {
                    $values11 = (floatval($row->g2pump_press_max) / floatval($rowmaster11->value));
                } else {
                    $values11 = floatval($row->g2pump_press_max);
                }
            ...
            $objWorkSheet11->setTitle('G2 Pump Pressure Maximal');

            // PTO Temperature Maximum
            $column12    = 0;
            $excel_row12 = 2;
            ...
            list($rowmaster12) = $this->mod_warning_exca->fetch_data_mastervar('pto_temp_max');
            foreach ($trend_data as $row){
                if ($rowmaster12->operation == 'multiplication') {
                    $values12 = (floatval($row->pto_temp_max) * floatval($rowmaster12->value));
                } elseif ($rowmaster12->operation == 'division') {
                    $values12 = (floatval($row->pto_temp_max) / floatval($rowmaster12->value));
                } else {
                    $values12 = floatval($row->pto_temp_max);
                }
            ...
            $objWorkSheet12->setTitle('PTO Temperature Maximum');

            // PTO Temperature Minimal
            $column13    = 0;
            $excel_row13 = 2;
            ...
            list($rowmaster13) = $this->mod_warning_exca->fetch_data_mastervar('pto_temp_min');
            foreach ($trend_data as $row){
                if ($rowmaster13->operation == 'multiplication') {
                    $values13 = (floatval($row->pto_temp_min) * floatval($rowmaster13->value));
                } elseif ($rowmaster13->operation == 'division') {
                    $values13 = (floatval($row->pto_temp_min) / floatval($rowmaster13->value));
                } else {
                    $values13 = floatval($row->pto_temp_min);
                }
            ...
            $objWorkSheet13->setTitle('PTO Temperature Minimal');
            ...
            list($rowmaster14) = $this->mod_warning_exca->fetch_data_mastervar('arm_ppc_on');
            foreach ($trend_data as $row){
                if ($rowmaster14->operation == 'multiplication') {
                    $values14 = (floatval($row->arm_ppc_on) * floatval($rowmaster14->value));
                } elseif ($rowmaster14->operation == 'division') {
                    $values14 = (floatval($row->arm_ppc_on) / floatval($rowmaster14->value));
                } else {
                    $values14 = floatval($row->arm_ppc_on);
                }
            ...
            $objWorkSheet14->setTitle('Arm PPC ON');
            ...
            list($rowmaster15) = $this->mod_warning_exca->fetch_data_mastervar('bucket_ppc_on');
            foreach ($trend_data as $row){
                if ($rowmaster15->operation == 'multiplication') {
                    $values15 = (floatval($row->bucket_ppc_on) * floatval($rowmaster15->value));
                } elseif ($rowmaster15->operation == 'division') {
                    $values15 = (floatval($row->bucket_ppc_on) / floatval($rowmaster15->value));
                } else {
                    $values15 = floatval($row->bucket_ppc_on);
                }
            ...
            $objWorkSheet15->setTitle('Bucket PPC ON');
            ...
            list($rowmaster16) = $this->mod_warning_exca->fetch_data_mastervar('boom_ppc_on');
            foreach ($trend_data as $row){
                if ($rowmaster16->operation == 'multiplication') {
                    $values16 = (floatval($row->boom_ppc_on) * floatval($rowmaster16->value));
                } elseif ($rowmaster16->operation == 'division') {
                    $values16 = (floatval($row->boom_ppc_on) / floatval($rowmaster16->value));
                } else {
                    $values16 = floatval($row->boom_ppc_on);
                }
            ...
            $objWorkSheet16->setTitle('Boom PPC ON');
            ...
            list($rowmaster17) = $this->mod_warning_exca->fetch_data_mastervar('travel_ppc_on');
            foreach ($trend_data as $row){
                if ($rowmaster17->operation == 'multiplication') {
                    $values17 = (floatval($row->travel_ppc_on) * floatval($rowmaster17->value));
                } elseif ($rowmaster17->operation == 'division') {
                    $values17 = (floatval($row->travel_ppc_on) / floatval($rowmaster17->value));
                } else {
                    $values17 = floatval($row->travel_ppc_on);
                }
            ...
            $objWorkSheet17->setTitle('Travel PPC ON');
            ...
            list($rowmaster18) = $this->mod_warning_exca->fetch_data_mastervar('swing_ppc_on');
            foreach ($trend_data as $row){
                if ($rowmaster18->operation == 'multiplication') {
                    $values18 = (floatval($row->swing_ppc_on) * floatval($rowmaster18->value));
                } elseif ($rowmaster18->operation == 'division') {
                    $values18 = (floatval($row->swing_ppc_on) / floatval($rowmaster18->value));
                } else {
                    $values18 = floatval($row->swing_ppc_on);
                }
            ...
            $objWorkSheet18->setTitle('Swing PPC ON');
