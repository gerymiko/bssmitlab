    ...
    class Sysmodule extends CI_Controller{
        ...
        function __construct(){
            ...
            if ($this->session->userdata('idu') == null && $this->session->userdata('nik') == null){
                ...
                $this->accessRights = $this->mod_global->get_access_rights($this->session->userdata('idu'), $this->uri->segment(3), $this->pregReps('master_modul'));
        ...
        public function table_module($site){
            ...
            foreach ($module as $field){
                if ($this->accessRights->id_level == 1) {
                    $btn = '<button class="btn btn-xs btn-default" data-toggle="modal" data-target="#modal-edit-module" data-tooltip="Edit" data-id_module="'.$this->my_encryption->encode($field->id_module).'" data-id_system="'.$field->id_system.'" data-name="'.$field->module_name.'" data-alias="'.$field->alias.'" data-desc="'.$field->description.'" data-active="'.$field->status_active.'" data-type="'.$field->type.'"><i class="fas fa-pen f10"></i></button>
                        <a class="btn btn-xs bg-red" data-tooltip="Delete" onclick="removeData(\''.$this->my_encryption->encode($field->id_module).'\', \''.$field->module_name.'\');"><i class="fas fa-times"></i></a>';
                } else {
        ...
        public function save_add_module($site){
            ...
            $Save = $this->mod_global->insert_all('mst_system_module', $data);
            if ($Save == true ) {
                $dataLog = array(
        ...
        public function save_edit_module($site){
            ...
            $Edit = $this->mod_global->edit_all('id_module', $id_module, 'mst_system_module', $data);
            if ($Edit == true ) {
                $dataLog = array(
                    'id_user'    => $this->accessRights->id_user,
                    'id_module'  => $this->accessRights->id_module,
        ...
        public function delete_module($site){
            if ($this->accessRights->deletex !== 1 || $this->accessRights->status_active !== 1){
                ...
                $isDelete = $this->mod_global->edit_all('id_module', $id_module, 'mst_system_module', $data);
                if ($isDelete == true){
                    $dataLog = array(
                        'id_user'    => $this->accessRights->id_user,
                        'id_module'  => $this->accessRights->id_module,
