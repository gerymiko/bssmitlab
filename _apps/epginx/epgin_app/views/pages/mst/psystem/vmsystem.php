			...
			<div class="box-tools pull-right">
				<?php
					if ($accessRights->id_level == 1) {
						echo '<button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-add-system"><i class="fas fa-plus"></i></button>';
					} else {
...
<script type="text/javascript">
    $(document).ready(function (){
    	...
    	$('#master-treeview, #link_master_sistem').addClass('active');
    	$('._CalPhaNum').alphanum({ allowNumeric: true, allow: '._-' });
   		...
   		var table = $('#table_system').DataTable({
			...
			"order": [],
			"ajax": {
			"url": '<?=site_url()?>sistem/t_sistem/<?=$accessRights->site?>',
			"type": 'POST',
