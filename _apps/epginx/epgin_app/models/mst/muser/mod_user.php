<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_user extends CI_Model {
        ...
        private function _get_user(){
	    	$this->db->select('c.name, a.id_user, a.idx, d.id_level, a.nik, a.fullname, a.email, a.phone, a.jabatan, a.kodest, a.status_active, MAX(b.insert_time) as login_time');
	        $this->db->from('mst_user a');
	        $this->db->join('mst_user_log b', 'b.id_user = a.id_user', 'left');
	        $this->db->join('mst_level_dtl c', 'a.idx = c.idx', 'inner');
	        $this->db->join('mst_level_hdr d', 'c.id_level = d.id_level', 'inner');
	        $this->db->join('HRD.dbo.TKaryawan e', 'a.nik = e.NIK AND e.AKTIF = 0', 'inner');
	        $this->db->group_by('c.name, a.id_user, a.idx, d.id_level, a.nik, a.fullname, a.email, a.phone, a.jabatan, a.kodest, a.status_active');
	        $i = 0;
	    ...
	    function get_module_user($id_user){
	    	...
	    	$query = $this->db->select('b.id, c.name, c.description, b.site, b.createx, b.readx, b.updatex, b.deletex, b.importx')
	    		->from('mst_user a')
	    		->join('mst_user_module b', 'a.id_user = b.id_user AND b.status_active = 1', 'inner')
	    		->join('mst_system_module c', 'b.id_module = c.id_module AND c.status_active = 1 AND c.isDelete = 0', 'inner')
	    		->where($datax)
	    		->get()
	    		->result();
	    	return $query;
	    ...
	    function get_module_user_based_access($id_user, $id_module, $site){
	    	...
	    	$query = $this->db->select('b.id_module, b.createx, b.readx, b.updatex, b.deletex, b.importx')
	    		->from('mst_user a')
	    		->join('mst_user_module b', 'a.id_user = b.id_user AND b.status_active = 1', 'inner')
	    		->join('mst_system_module c', 'b.id_module = c.id_module AND c.status_active = 1 AND c.isDelete = 0', 'inner')
	    		->where($datax)
		...
		function get_employee($nik){
	    	...
	    	$query = $this->db->select('a.NIK, a.Nama, a.Telp, a.Email, a.KodeST, b.Nama as jabatan, a.KodeDP')
	    ...
	    function list_level(){
	    	$datax = array('a.status_active' => 1, 'b.status_active' => 1 );
	    	$query = $this->db->select('b.id_level, b.name, b.status_active')
    			->from('mst_level_hdr a')
    			->join('mst_level_dtl b', 'a.id_level = b.id_level', 'inner')
    			->where($datax)
    			->get()
    			->result();
	    	return $query;
