<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mod_master_raport_loading extends CI_Model {
	    ...
	    function list_parameter_hdr($site){
			...
			$query = $this->db->select('id, parameter')
				->from('mst_parameter_raport_loading_hdr')
				->where($datax)
				->get()
				->result();
			return $query;
	    ...
	    private function _get_data_hdr($site){
	        ...
	        $this->db->from('mst_parameter_raport_loading_hdr');
	        $this->db->where($datax);
	        $i = 0;
	    ...
	    private function _get_data_dtl($site){
	    	...
	    	$this->db->select('b.id, b.id_parameter_raport_loading_hdr,  a.parameter, b.batas_atas, b.batas_bawah, b.dynamic_value, b.nilai, b.status_active');
	        $this->db->from('mst_parameter_raport_loading_hdr a');
	        $this->db->join('mst_parameter_raport_loading_dtl b', 'a.id = b.id_parameter_raport_loading_hdr', 'inner');
	        $this->db->where($datax);
	        $i = 0;
