<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysmonitor extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['mrecruit/web/monitor/mod_monitor']);
        }
        ...
        public function table_monitor(){
            ...
            $monitorRecap = $this->mod_monitor->get_datatables($length, $start);
            foreach ($monitorRecap as $field){
                ...
                if ($field->lowongan_status == 1) {
                    $vacanStatus = '<i class="fas fa-circle text-green"></i>';
                } else {
                    $vacanStatus = '<i class="fas fa-circle text-red"></i>';
                }
                $row         = array();
                $row['no']   = $start;
                ...
                $row['action']  = '
                    <a class="hand btn btn-primary btn-xs red-tooltip" data-toggle="tooltip" title="Detail" onclick="detailApplicant(\''.$this->my_encryption->encode($field->people_id).'\')" >
