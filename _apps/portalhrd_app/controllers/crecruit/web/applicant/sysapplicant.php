    ...
    class Sysapplicant extends CI_Controller {
        ...
        public function table_applicant(){
            ...
            foreach ($applicant as $field){
                ...
                $kspm    = ($tahap_kspm !== false) ? '<button class="btn btn-xs bg-blue" data-toggle="modal" data-target="#modal-sms-kspm" data-pid="'.$this->my_encryption->encode($field->people_id).'" data-plid="'.$this->my_encryption->encode($field->pelamar_id).'" data-name="'.$field->people_fullname.'" data-mobile="'.$field->people_mobile.'"><i class="fas fa-envelope"></i> SMS</button>' : '<span class="label bg-gray"><em>Tidak ada tahap KSPM & HRD</em></span>';
                $teknis  = ($tahap_teknis !== false) ? '<button class="btn btn-xs bg-blue" data-toggle="modal" data-target="#modal-sms-teknis" data-pid="'.$this->my_encryption->encode($field->people_id).'" data-plid="'.$this->my_encryption->encode($field->pelamar_id).'" data-name="'.$field->people_fullname.'" data-mobile="'.$field->people_mobile.'"><i class="fas fa-envelope"></i> SMS</button>' : '<span class="label bg-gray"><em>Tidak ada tahap Teknis</em></span>';
                $teori   = ($tahap_teori !== false) ? '<button class="btn btn-xs bg-blue" data-toggle="modal" data-target="#modal-sms-teori" data-pid="'.$this->my_encryption->encode($field->people_id).'" data-plid="'.$this->my_encryption->encode($field->pelamar_id).'" data-name="'.$field->people_fullname.'"><i class="fas fa-envelope"></i> SMS</button>' : '<span class="label bg-gray"><em>Tidak ada tahap Tes Teori</em></span>';
                $praktek = ($tahap_praktek !== false) ? '<button class="btn btn-xs bg-blue" data-toggle="modal" data-target="#modal-sms-praktek" data-pid="'.$this->my_encryption->encode($field->people_id).'" data-plid="'.$this->my_encryption->encode($field->pelamar_id).'" data-name="'.$field->people_fullname.'"><i class="fas fa-envelope"></i> SMS</button>' : '<span class="label bg-gray"><em>Tidak ada tahap Tes Praktek</em></span>';
                $mcu     = ($tahap_mcu !== false) ? '<button class="btn btn-xs bg-blue" data-toggle="modal" data-target="#modal-sms-mcu" data-pid="'.$this->my_encryption->encode($field->people_id).'" data-plid="'.$this->my_encryption->encode($field->pelamar_id).'" data-name="'.$field->people_fullname.'"><i class="fas fa-envelope"></i> SMS</button>' : '<span class="label bg-gray"><em>Tidak ada tahap MCU</em></span>';
                ...
                $row['no']  = $start;
                $row['sms'] = '
                    <a class="btn btn-danger btn-xs details-control" data-tooltip="SMS Interview">
