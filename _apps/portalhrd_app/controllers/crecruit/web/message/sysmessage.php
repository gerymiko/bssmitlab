<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Sysmessage extends CI_Controller {
        ...
        function __construct() {
            ...
            $this->load->model(['mrecruit/web/message/mod_message']);
        }
        ...
        public function send_sms_kspm(){
            ...
            $people_mobile  = $this->pregRepn($this->input->post('mobile'));
            ...
            if ($checkSchedule !== false) { echo "Duplicate";exit(); }
            $msgid          = 2;
            ...
            if($people_id !== '' && $pelamar_id !== ''){
                ...
                $saveSchedule = $this->mod_global->insert_all('schedule', $dataMessage);
        ...
        public function send_sms_teknis(){
            $people_id      = $this->my_encryption->decode($this->pregReps($this->input->post('people_id')));
            $pelamar_id     = $this->my_encryption->decode($this->pregReps($this->input->post('pelamar_id')));
            $people_mobile  = $this->pregReps($this->input->post('mobile'));
            $interview_date = $this->pregReps($this->input->post('interview_date'));
            $interview_time = $this->pregReps($this->input->post('interview_time'));
        ...
        public function send_sms_mcu(){
            ...
            if($people_id !== '' && $pelamar_id !== ''){
                ...
                if ($saveSchedule == true) {
                    $dataPJV = array('mcu' => 2);
                    $savePJV = $this->mod_global->edit_all('jv_id', $getPJV->jv_id, 'parameter_job_vacancy', $dataPJV);
                    if ($savePJV == true) {
                        $pasienMCU = array(
                            'clinic_id'  => $clinic_id,
                            'pelamar_id' => $pelamar_id,
                        ...
                        $savePatient = $this->mod_global->insert_all('WEB.dbo.patient_mcu', $pasienMCU);
                        if ($savePatient == true) {
                            $dataSite = array( 'KodeST' => $site );
                            $saveSite = $this->mod_global->edit_all('pelamar_id', $pelamar_id, 'pelamar', $dataSite);
                            if ($saveSite == true) {
                                $konten = array(
                                    'NOM' => $people_mobile,
                                    'MSG' => $content
