...
<script type="text/javascript">
	...
	function removeData(id, name){
	    swal({
	        title: "",
	        html: '<i class="fas fa-question-circle f40 margin10 text-red"></i><br>Nonaktifkan user ini (<b>'+name+'</b>).',
	        type: "",
			...
			if (result.value) {
			    $.ajax({
					url: "<?=site_url()?>snon/user",
					type: "post",
					data: { id:id },
					success:function(data){
						if(data == "Success"){
							swal({ title: "", html: '<i class="fas fa-check-circle f40 margin10 text-green"></i><br>User berhasil dinonaktifkan.', type: "",
						        confirmButtonText: 'Okay'});
						    $('#table_user').DataTable().ajax.reload();
						} else {
							swal({ title: "", html: '<i class="fas fa-times-circle f40 margin10 text-red"></i><br>Gagal dinonaktifkan. Muat ulang halaman ini dan coba lagi.', type: "", confirmButtonText: 'Okay'});
						}
					},
				});
			}
