<section class="content">
   <div class="box box-primary">
      <div class="box-header with-border">
         <h3 class="box-title text-bold">Tambah Lowongan</h3>
...
<script type="text/javascript">
   $(document).ready(function(){
      ...
      $('#jabatan_alias').change(function(){
         ...
         $.ajax({
            type : "POST",
            url  : "<?=site_url()?>web/check/vacancy",
            cache: false,    
            data:'jabatan_alias=' + $("#jabatan_alias").val(),
            success: function(response){
               try {
                  if(response == "false"){ $(".loadloker").hide();$("#btn_add_vacancy").addClass('hidden');$("#availability-loker").show();
                  } else { $(".loadloker").hide(); $("#availability-loker").hide(); $("#btn_add_vacancy").removeClass('hidden');}         
               } catch(e){
                  swal({ title: "", html: '<i class="fas fa-exclamation-circle f40 margin10 text-orange"></i><br>Terjadi kesalahan. Reload halaman ini.', type: "", confirmButtonText: 'Okay' });
               }
            },
         });
      });
      $('#form-add-vacancy').validate({
         rules: { 'edutype_id[]': { required: true } ,'skill_id[]': { required: true } ,'syarat_id[]': { required: true } },
         messages: { 'edutype_id[]': { required: "Minimal pilih salah satu opsional", }, 'skill_id[]': { required: "Minimal pilih salah satu opsional", }, 'syarat_id[]': { required: "Minimal pilih salah satu opsional", }}         
      });
       $('#jabatan_alias, #jabatan_edit').change(function(){
         var opt = 'skill=' + $(this).val();
         $.ajax({
            type: "POST",
            url: "<?=site_url()?>web/get/skill",
            data: opt,
            success: function(data){ $("#skill").html(data);}
         });
      });
      $('#jabatan_alias, #jabatan_edit').change(function(){
         ...
         $.ajax({
            type: "POST",
            url: "<?=site_url()?>web/get/certificate",
            data: opt,
            success: function(data){ $("#sertifikat").html(data);
            }
         });
      });
      $('#jabatan_alias, #jabatan_edit').change(function(){
         ...
         $.ajax({
            type: "POST",
            url: "<?=site_url()?>web/get/condition",
            data: opt,
      ...
      $("#btn_add_vacancy").click(function(){
         ...
         if($("#form-add-vacancy").valid() == false){
            ...
            $.post("<?=base_url();?>web/add/vacancy",
            formdata,
            function(data) {
               if(data == "Success"){
                  $("#loading").addClass("hidden");
                  $('#modal-add-vacancy').modal('hide');
                  swal({ title: "", html: '<i class="fas fa-check-circle f40 margin10 text-green"></i><br>Data berhasil disimpan.', type: "", confirmButtonText: 'Okay', allowOutsideClick: false }).then(function(){ table.ajax.reload(); });
               } else if (data == 'duplicate') {
                  $("#loading").addClass("hidden");
                  $('#modal-add-vacancy').modal('hide');
                  swal({ title: "", html: '<i class="fas fa-info-circle f40 margin10 text-orange"></i><br>Duplikat data, cek apakah data sudah terdaftar atau belum.', type: "", confirmButtonText: 'Okay' });
               } else {
                  $("#loading").addClass("hidden");
                  $('#modal-add-vacancy').modal('hide');
