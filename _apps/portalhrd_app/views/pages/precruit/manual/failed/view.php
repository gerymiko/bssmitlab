<section class="content-header" id="header-content">
	<h1><span class="label no-padding text-black">Data Pelamar Gagal</span></h1>
</section>
...
<section class="content" id="main-content">
	...
	<div class="box">
		<div class="box-body slide-content">
			<table id="table_applicant_failed" class="table table-border table-striped table-hover" width="100%">
				<thead>
					<tr>
						<th>No</th>
						<th></th>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		var table = $('#table_applicant_failed').DataTable({
			...
			"order": [],
			"dom": 'Bfrtip',
	        "buttons": [ 'pageLength', { extend : 'excel', text : 'Export Excel', action: newExportAction }, { text: 'Reload', action: function (e, dt, node, config){ table.ajax.reload();}}],
	        "lengthMenu": [ [10, 25, 50, 100],['10 Baris', '25 Baris', '50 Baris', '100 Baris'] ],
			"ajax": {
				"url": '<?=site_url()?>form/table/failed',
				"type": 'POST',
				"data" : function(data){ data.people_fullname = $('#people_fullname').val();data.KodeJB = $('#KodeJB').val();data.interview_site = $('#interview_site').val();data.date_range = $('#date_range').val();data.interview_status = $('#interview_status').val();},
			...
			"columns": [
				...
				{ "data": "domisili", "className": "text-center", "searchable": false, "visible": false, "orderable": false },
				{ "data": "position", "className": "text-left", "orderable": false },
				{ "data": "date", "className": "text-center" },
				{ "data": "site", "className": "text-center", "orderable": false },
				{ "data": "stage", "className": "text-center", "orderable": false },
	...
	function detailApplicant(id){
		...
		$("#extra-content").load("<?=site_url()?>form/detail/applicant/"+id, function(){
			$(".select2").select2({ placeholder: "Pilih", allowClear: true });
	...
	function detailInterview(id){
		$("#main-content, #header-content").addClass("hidden");
		$("#extra-content").removeClass("hidden");
		$("#extra-content").load("<?=site_url()?>form/detail/interview/"+id, function(){
			$(".select2").select2({ placeholder: "Pilih", allowClear: true });
			$('.data-mask').inputmask('dd-mm-yyyy');
			$('.modal').on('hidden.bs.modal', function(){ var modal = $(this);modal.validate().resetForm();modal.find('.error').removeClass('error'); });
		});
	...
	function btn_reinterview(id, name, period){
	    swal({
			...
			if (result.value) {
			    $.ajax({
					url: "<?=site_url()?>form/re/interview",
					type: "post",
					data: {id:id},
					success:function(result){
						if (result == "Success"){
							swal({ title: "", html: '<i class="fas fa-check-circle f40 margin10 text-green"></i><br>Pelamar berhasil diaktivasi (<b>'+name+'</b>).', type: "", confirmButtonText: 'Okay'});
							...
							swal({ title: "", html: '<i class="fas fa-exclamation-circle f40 margin10 text-orange"></i><br>Maaf anda belum bisa melakukan interview kembali pada pelamar (<b>'+name+'</b>), karena masa aktif (<b>+6 bulan</b>) belum terlewati.<br>Tanggal aktif (<b>'+period+'</b>)', type: "", confirmButtonText: 'Okay, Saya mengerti' });
						} else if (result == "StillRunning"){
							swal({ title: "", html: '<i class="fas fa-exclamation-circle f40 margin10 text-orange"></i><br>Maaf interview kembali tidak dapat dilakukan pada pelamar (<b>'+name+'</b>) karena sedang diproses.', type: "", confirmButtonText: 'Okay, Sorry' });
						} else {
							swal({ title: "", html: '<i class="fas fa-exclamation-circle f40 margin10 text-red"></i><br>Gagal memproses. Reload halaman dan coba lagi.', type: "", confirmButtonText: 'Okay, Sorry' });
						}
					},
				});
			}
					...
					success:function(data){
						if(data == "Success"){
							swal({ title: "", html: '<i class="fas fa-check-circle f40 margin10 text-green"></i><br>Data berhasil dihapus.', type: "",
						        confirmButtonText: 'Okay' }).then(function(){ $('#table_applicant_failed').DataTable().ajax.reload();});
						} else {
