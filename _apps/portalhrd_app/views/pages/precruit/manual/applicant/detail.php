<section class="content-header" id="header-content">
	<h1>
		<span class="label no-padding text-black">Detail Pelamar 
	...
	<div class="row">
		<div class="col-md-12">
			<ul class="timeline">
				...
				<li>
					<i class="fas fa-bullseye text-navy"></i>
					<div class="timeline-item">
						<span class="time" data-tooltip="Tambah Data" style="padding: 8px;"><a class="btn btn-primary btn-xs pull-right" data-toggle="modal" data-target="#modal-add-lisence" data-backdrop="static" data-keyboard="false" data-id="<?=$this->my_encryption->encode($dapplicant->people_id)?>"><i class="fas fa-plus"></i></a></span>
						<h3 class="timeline-header">Identitas dan Surat Izin Mengemudi</h3>
						<div class="timeline-body">
							<div class="row">
								<?php
									foreach ($dlisence as $row) {
										if ($row->lisence_type == 'KTP') {
											$btn_hide = "hidden";
										} else {
											$btn_hide = "";
										}
										...
										<div class="col-md-4">
											<div class="pull-right">
												<button class="btn bg-gray btn-xs" data-tooltip="Ubah" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#modal-edit-lisence" data-lisence_id="'.$this->my_encryption->encode($row->plisence_id).'" data-lisence_number="'.$row->lisence_number.'" data-lisence_period="'.$datePeriod.'" data-lisence_type="'.$row->lisence_type.'" ><i class="fas fa-pen f10"></i></button>
												<button class="btn btn-danger btn-xs '.$btn_hide.' " data-tooltip="Hapus" onclick="removeThis(\''.$this->my_encryption->encode($row->plisence_id).'\', \''.$row->lisence_type.'\', \'delete_lisence\')"><i class="fas fa-times"></i></button>
											</div>
				...
				<li>
					...
					<div class="timeline-item">
						<span class="time" data-tooltip="Tambah" style="padding: 8px;"><a class="btn btn-primary btn-xs pull-right" data-toggle="modal" data-target="#modal-add-experience" data-backdrop="static" data-keyboard="false" data-id="<?=$this->my_encryption->encode($dapplicant->people_id)?>"><i class="fas fa-plus"></i></a></span>
									...
									foreach ($dexperience as $row) {
										...
										<div class="col-md-4">
											<div class="pull-right">
												...
												<button class="btn btn-danger btn-xs" data-tooltip="Hapus" onclick="removeThis(\''.$this->my_encryption->encode($row->pexp_id).'\', \''.$row->company_name.'\', \'delete_experience\')"><i class="fas fa-times"></i></button>
											</div>
				...
				<li>
					<i class="fas fa-baseball-ball text-navy"></i>
					<div class="timeline-item">
						<span class="time" data-tooltip="Tambah" style="padding: 8px;"><a class="btn btn-primary btn-xs pull-right" data-toggle="modal" data-target="#modal-add-skill" data-backdrop="static" data-keyboard="false" data-id="<?=$this->my_encryption->encode($dapplicant->people_id)?>"><i class="fas fa-plus"></i></a></span>
								...
								<?php
									...
									if ($countskill !== 0) {
										...
										foreach ($dskill as $row) {
											...
											<div class="col-md-3">
												<div class="pull-right">
													<button class="btn bg-gray btn-xs" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#modal-edit-skill" data-pskill_id="'.$this->my_encryption->encode($row->pskill_id).'" data-skill_name="'.$row->skill_name.'" data-skill_unit="'.$row->skill_unit.'"><i class="fas fa-pen f10"></i></button> 
													<button class="btn btn-danger btn-xs" data-tooltip="Hapus" onclick="removeThis(\''.$this->my_encryption->encode($row->pskill_id).'\', \''.$row->skill_name.'\', \'delete_skill\')"><i class="fas fa-times"></i></button>
												</div>
							...
							<table class="table table-hover">
								<tbody>
									<tr>
										...
										<th class="text-center">Tgl Melamar</th>
										<th class="text-center">Tahap Terakhir</th>
										<th>Keterangan</th>
									</tr>
									<?php
											...
											foreach ($dinterview as $row){
												...
												echo '
													<tr>
														...
														<td>'.$row->jabatan.'</td>
														<td class="text-center">'.$row->interview_site.'</td>
														<td class="text-center">'.date("d-m-Y", strtotime($row->tgl_melamar)).'</td>
	...
	<div class="modal-dialog modal700">
		<div class="modal-content">
			...
			<form id="form_edit_pelamar" action="#" method="post">
				<input type="hidden" name="people_id" id="people_id_edit">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form id="form_edit_lisence" action="#" method="post">
				...
				<input type="hidden" name="lisence_type" id="lisence_type_edit">
				<div class="modal-body">
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label">Nomor</label>
								<input type="text" maxlength="18" name="lisence_number" id="lisence_number_edit" class="form-control _CnUmB required" placeholder="Ketik disini">
							</div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
		                       	...
		                       	<div class="input-group date">
		                          	...
		                          	<input type="text" class="form-control data-mask required pull-right" name="lisence_period" id="lisence_period_edit" maxlength="10" placeholder="dd-mm-yyyy"/>
		                       	</div>
		                    </div>
						</div>
					</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form id="form_add_lisence" action="#" method="post">
				<input type="hidden" name="people_id" id="people_id_lisence">
				<div class="modal-body">
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label">Jenis SIM</label>
								<select name="lisence_type" class="form-control select2 required">
					                <option></option>
							...
							<div class="form-group">
		                       	...
		                       	<div class="input-group date">
		                          	<div class="input-group-addon"> <i class="far fa-calendar-alt"></i></div>
		                          	<input type="text" class="form-control data-mask required pull-right" name="lisence_period" maxlength="10" placeholder="dd-mm-yyyy"/>
		                       	</div>
		                    </div>
						</div>
						<div class="col-md-6">
							<div class="form-group">
								<label class="control-label">Nomor</label>
								<input type="text" maxlength="18" name="lisence_number" class="form-control _CnUmB required" placeholder="Ketik disini">
							</div>
						</div>
					</div>
					<small>*Perubahan terhadap data ini akan masuk dalam <b>log aplikasi</b>.</small>				
	...
	<div class="modal-dialog modal900">
		<div class="modal-content">
			...
			<form id="form_add_experience" action="#" method="post">
				<input type="hidden" name="people_id" id="people_id_exp">
				<div class="modal-body">
					<div class="row">
						<div class="col-md-4">
							<div class="form-group">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form id="form_add_skill" action="#" method="post">
				<input type="hidden" name="people_id" id="people_id_skill">
				<div class="modal-body">
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#btn_add_skill_more').click(function(e){ 
			...
			if(y < 5){ 
				...
				swal({ title: "", html: '<i class="fas fa-info-circle f40 margin10 text-orange"></i><br>Batas kemampuan hanya 6 kolom.', type: "", confirmButtonText: 'Okay' });
			}
		});
		...
		$('#btn_add_exp_more').click(function(e){ 
			...
			if(x < 5){ 
				...
				swal({ title: "", html: '<i class="fas fa-info-circle f40 margin10 text-orange"></i><br>Batas pengalaman hanya 6 kolom.', type: "", confirmButtonText: 'Okay' });
			}
		});
		...
		$('#modal-edit-applicant').on('show.bs.modal', function (event) {
			if (event.namespace == 'bs.modal') {
				...
				var modal   = $(this)
				modal.find('#people_id_edit').val(people_id)
		...
		$("#btn_sv_applicant").click(function (){
			...
			if($("#form_edit_pelamar").valid() == false){
				...
				$.post("<?=base_url();?>form/edit/personal",
				formdata,
				function(data){
					if(data == "Success"){
						$("#loading").addClass("hidden");
						...
						swal({
					        title: "", html: '<i class="fas fa-check-circle f40 margin10 text-green"></i><br>Data berhasil disimpan.', type: "",
					        confirmButtonText: 'Okay', allowOutsideClick: false }).then(function(){
							$("#main-content, #header-content").removeClass("hidden");
							$("#extra-content").addClass("hidden");
						...
						$("#loading").addClass("hidden");
						$('#modal-edit-applicant').modal('hide');
						swal({ title: "", html: '<i class="fas fa-info-circle f40 margin10 text-orange"></i><br>Tidak ada perubahan data.', type: "",
		...
		$("#btn_sv_address").click(function (){
			...
			if($("#form_edit_address").valid() == false){
				...
				$.post("<?=base_url();?>form/edit/address",
				formdata,
				function(data){
					if(data == "Success"){
						$("#loading").addClass("hidden");
		...
		$('#modal-add-lisence').on('show.bs.modal', function (event) {
			if (event.namespace == 'bs.modal') {
				...
				var modal  = $(this)
				modal.find('#people_id_lisence').val(id)
			}
		});
		...
		$('#modal-edit-lisence').on('show.bs.modal', function (event) {
			if (event.namespace == 'bs.modal') {
				...
				modal.find('#lisence_number_edit').val(lisence_number)
				modal.find('#lisence_period_edit').val(lisence_period)
				modal.find('#lisence_type_edit').val(lisence_type)
				if (lisence_type == "KTP"){
		...
		$("#btn_sv_lisence").click(function (){
			...
			if($("#form_edit_lisence").valid() == false){
				...
				$.post("<?=base_url();?>form/edit/lisence",
				formdata,
				function(data){
					if(data == "Success"){
						$("#loading").addClass("hidden");
		...
		$("#btn_svadd_lisence").click(function (){
			...
			if($("#form_add_lisence").valid() == false){
				...
				$.post("<?=base_url();?>form/add/lisence",
				formdata,
				function(data){
					if(data == "Success"){
						$("#loading").addClass("hidden");
		...
		$("#btn_sv_experience").click(function (){
			...
			if($("#form_edit_experience").valid() == false){
				...
				$.post("<?=base_url();?>form/edit/experience",
				formdata,
				function(data){
					if(data == "Success"){
						$("#loading").addClass("hidden");
		...
		$('#modal-add-experience').on('show.bs.modal', function (event) {
			if (event.namespace == 'bs.modal') {
				...
				var id     = button.data('id')
				var modal  = $(this)
				modal.find('#people_id_exp').val(id)
			}
		});
		...
		$("#btn_svadd_experience").click(function (){
			...
			if($("#form_add_experience").valid() == false){
				...
				$.post("<?=base_url();?>form/add/experience",
				formdata,
				function(data){
					if(data == "Success"){
						$("#loading").addClass("hidden");
		...
		$('#modal-add-skill').on('show.bs.modal', function (event) {
			if (event.namespace == 'bs.modal') {
				...
				var id     = button.data('id')
				var modal  = $(this)
				modal.find('#people_id_skill').val(id)
			}
		});
	...
	function removeThis(id, name, funct){
	    swal({
			...
			if (result.value) {
			    $.ajax({
					url: "<?=site_url()?>form/delete/"+funct,
					type: "post",
					data: { id:id },
